use crate::actionsapi as luppo;
use crate::sandbox::SandboxContext;
use indicatif::{ProgressBar, ProgressStyle};
use luppo_core::colorize;
use rayon::prelude::*;
use luppo_core::config::Config;
use luppo_core::database::LuppoDatabase;
use luppo_core::packager::Packager;
use luppo_core::repo::Repository;
use luppo_core::resolver::resolve_build_deps_static;
...
pub struct PackageBuilder {
    spec: LuppoSpec,
    /// pkg_dir: /var/luppo/<name>-<ver>-<rel>/
    work_dir: PathBuf,
    /// specdir: lopec.xml veya .kdl dosyasının bulunduğu dizin
    specdir: PathBuf,
    db: LuppoDatabase,
    config: Config,
...
impl PackageBuilder {
    ...
    /// Python Luppo'nin pkg_work_dir(): <pkg_dir>/work/ veya <pkg_dir>/work-<build_type>/
    ...
    /// Python Luppo'nin pkg_install_dir(): <pkg_dir>/install/
    fn pkg_install_dir(&self) -> PathBuf {
        self.work_dir.join("install")
    }

    /// Build state dosyasının yolu: <pkg_dir>/work/luppoBuildState
    fn state_file_path(&self, build_type: &str) -> PathBuf {
        self.pkg_work_dir(build_type).join("luppoBuildState")
    }
    ...
    fn internal_build(
        ...
        luppo::cd(&self.work_dir)?;

        // 0. Çevre Değişkenlerini Seçeneklere Göre Ayarla
        luppo_core::safe_env::set_var("MAKEOPTS", format!("-j{}", self.options.jobs));
        luppo_core::safe_env::set_var("MAKEFLAGS", format!("-j{}", self.options.jobs));
        if self.options.verbose {
            luppo_core::safe_env::set_var("VERBOSE", "1");
        }
        if self.options.debug {
            luppo_core::safe_env::set_var("LUPPO_DEBUG", "1");
        }
        if let Some(ref level) = self.options.optimization_level {
            ...
            luppo_core::safe_env::set_var("CFLAGS", &opt_flag);
            luppo_core::safe_env::set_var("CXXFLAGS", &opt_flag);
            ...
            luppo_core::safe_env::set_var("LDFLAGS", format!("{} -Wl,-z,relro -Wl,--hash-style=gnu", ld_opt));
        }
        if let Some(ref log) = self.options.log_path {
            // actionsapi'nin log yolunu görebilmesi için çevre değişkeni ayarla
            luppo_core::safe_env::set_var("LUPPO_BUILD_LOG", log.to_string_lossy());
        }
        ...
        if !all_build_deps.is_empty() {
            ...
            if !unsatisfied.is_empty() {
                ...
                luppo_core::print_in_columns(&dep_names);
                ...
                if self.options.enable_sandbox {
                    ...
                    let installer = luppo_core::installer::Installer::new(
                        self.db.clone(),
                        self.config.clone(),
                    );
        ...
        if do_package {
            ...
            let results: Vec<Result<PathBuf, String>> = self.spec.packages.par_iter().enumerate().map(|(pkg_idx, pkg_def)| {
                ...
                crate::package::create_luppo_package(
                    &self.spec,
                    pkg_idx,
                    &pkg_install_root,
                    &self.work_dir,
    ...
    fn run_build_steps(
        ...
        luppo_core::safe_env::set_var("SRC_NAME", pkg_name);
        luppo_core::safe_env::set_var("SRC_VERSION", &pkg_version);
        luppo_core::safe_env::set_var("SRC_RELEASE", &pkg_release);

        let host = format!("{}-pc-linux-gnu", std::env::consts::ARCH);
        luppo_core::safe_env::set_var("HOST", &host);
        if std::env::var("CC").is_err() {
            luppo_core::safe_env::set_var("CC", format!("{}-gcc", &host));
        }
        if std::env::var("CXX").is_err() {
            luppo_core::safe_env::set_var("CXX", format!("{}-g++", &host));
        }
        if std::env::var("CFLAGS").is_err() {
            luppo_core::safe_env::set_var("CFLAGS", "-O2 -fomit-frame-pointer");
        }
        if std::env::var("CXXFLAGS").is_err() {
            luppo_core::safe_env::set_var("CXXFLAGS", "-O2 -fomit-frame-pointer");
        }
        if std::env::var("LDFLAGS").is_err() {
            luppo_core::safe_env::set_var("LDFLAGS", "-Wl,-O1 -Wl,-z,relro -Wl,--hash-style=gnu");
        }
        if std::env::var("JOBS").is_err() {
            luppo_core::safe_env::set_var("JOBS", luppo::get::make_jobs());
        }
        ...
        luppo_core::safe_env::set_var("LUPPO_SBINDIR", &self.options.sbindir);
        ...
        for bt in &build_types {
            ...
            luppo_core::safe_env::set_var("LUPPO_BUILD_TYPE", bt);

            let bt_work_dir = self.pkg_work_dir(bt);
            luppo_core::safe_env::set_var("WORK_DIR", bt_work_dir.to_str().unwrap_or("work"));
            luppo_core::safe_env::set_var("HOME", bt_work_dir.to_str().unwrap_or("work"));
            luppo_core::safe_env::set_var("PKG_DIR", self.work_dir.to_str().unwrap_or("/var/luppo"));
            luppo_core::safe_env::set_var("PYTHONDONTWRITEBYTECODE", "1");

            let cargo_home = bt_work_dir.join(".cargo");
            if !cargo_home.exists() {
                fs::create_dir_all(&cargo_home).map_err(|e| e.to_string())?;
            }
            luppo_core::safe_env::set_var("CARGO_HOME", cargo_home.to_str().unwrap_or(""));

            let install_root_bt = self.pkg_install_dir();
            luppo_core::safe_env::set_var("INSTALL_DIR", install_root_bt.to_str().unwrap_or("install"));

            // emul32 (32-bit) build type için derleyici ve pkg-config ortam değişkenlerini ayarla
            if bt == "emul32" {
                luppo_core::safe_env::set_var("HOST", "i686-pc-linux-gnu");
                luppo_core::safe_env::set_var("CHOST", "i686-pc-linux-gnu");
                luppo_core::safe_env::set_var("CC", "gcc -m32");
                luppo_core::safe_env::set_var("CXX", "g++ -m32");
                let cflags = std::env::var("CFLAGS").unwrap_or_default();
                if !cflags.contains("-m32") {
                    luppo_core::safe_env::set_var("CFLAGS", format!("{} -m32", cflags).trim());
                }
                let cxxflags = std::env::var("CXXFLAGS").unwrap_or_default();
                if !cxxflags.contains("-m32") {
                    luppo_core::safe_env::set_var("CXXFLAGS", format!("{} -m32", cxxflags).trim());
                }
                let ldflags = std::env::var("LDFLAGS").unwrap_or_default();
                if !ldflags.contains("-m32") {
                    luppo_core::safe_env::set_var("LDFLAGS", format!("{} -m32", ldflags).trim());
                }
                ...
                if !existing.contains(emul32_pkg) {
                    if existing.is_empty() {
                        luppo_core::safe_env::set_var("PKG_CONFIG_LIBDIR", emul32_pkg);
                    } else {
                        luppo_core::safe_env::set_var(
                            "PKG_CONFIG_LIBDIR",
                            format!("{}:{}", emul32_pkg, existing),
                        );
                    }
                }
                luppo_core::safe_env::set_var("PKG_CONFIG_SYSROOT_DIR", "/");
            }

            // Her döngü başında çalışma dizinini sıfırla, böylece
            // actions.py içindeki cd işlemleri bir sonraki döngüyü etkilemez
            luppo::cd(&bt_work_dir)?;
            ...
            if do_prepare || source_missing {
                ...
                for archive in &self.spec.source.archives {
                    ...
                    luppo::verify_archive(archive_name, &hash_val, &hash_type)?;
                    ...
                    for src_dir in &src_dirs {
                        if src_dir.exists() && src_dir.is_dir() {
                            ...
                            for (patch_path, level) in &patch_paths {
                                match luppo::do_patch(patch_path, *level) {
                                    Ok(_) => {
                                        spinner.println(colorize(
            ...
            if let Some(env_vars) = &self.spec.source.environment {
                for env in &env_vars.vars {
                    ...
                    if env.force || std::env::var(&env.name).is_err() {
                        luppo_core::safe_env::set_var(&env.name, val);
            ...
            if let Some(first_pkg) = bt_pkg {
                ...
                // Autoreconf/Intltool desteği için ACLOCAL_PATH ayarla (varsa koru)
                {
                    ...
                    luppo_core::safe_env::set_var("ACLOCAL_PATH", &dirs);
                }

                // Python Luppo: clean install dir before default build type install
                ...
                for (i, step) in first_pkg.actions.steps.iter().enumerate() {
                    ...
                    let mut cmd = step.replace("{jobs}", &luppo::get::make_jobs());
                    cmd = cmd.replace(
                        "{install_root}",
                        install_root_bt.to_str().unwrap_or("install_root"),
                    );
                    cmd = cmd.replace("{source_dir}", ".");
                    cmd = cmd.replace("{KERNEL_RELEASE}", &luppo::cur_kernel());
                ...
                pyo3::Python::with_gil(|py| -> Result<(), String> {
                    let actionsapi_mod =
                        PyModule::new(py, "luppo.actionsapi").map_err(|e| e.to_string())?;
                    ...
                    sys_modules
                        .set_item("luppo.actionsapi", actionsapi_mod)
                        .map_err(|e| e.to_string())?;
                    ...
                    for line in &lines {
                        ...
                        if trimmed.starts_with("from luppo.actionsapi") || trimmed.starts_with("import luppo") {
                            continue;
                        }
                        filtered.push(*line);
                    }
                    ...
                    for key in ["get", "shelltools", "luppotools", "autotools", "cmaketools", "mesontools", "pythonmodules", "python3modules", "qt5", "qt6", "kde6", "sconstools", "cargotools", "perlmodules", "kerneltools", "waftools", "anttools", "npmtools", "gotools", "libtools"] {
...
",
                ...
                if do_install && !self.is_state_done(bt, "install") {
                    ...
                    luppo::autotools_install(install_root_relative, &[])?;
                    self.write_state(bt, "install");
            ...
            if do_install {
                luppo::strip_dir(install_root_bt.to_str().unwrap_or("install"), &no_strip)?;
                luppo::fix_pkgconfig(install_root_bt.to_str().unwrap_or("install"))?;
                luppo::merge_usr_dirs(install_root_bt.to_str().unwrap_or("install"))?;
            }

            // Build type döngüsü sonunda çevre değişkenlerini sıfırla (emul32 sızıntısını önle)
            for (key, val) in &initial_env {
                luppo_core::safe_env::set_var(key, val);
            }
        }
    ...
    fn write_debug_log(&self, error: &str) {
        let log_path = self
            ...
            .log_path
            .clone()
            .unwrap_or_else(|| self.work_dir.join("luppo-build-error.log"));
    ...
    /// /etc/luppo/mirrors.conf (ve cwd/mirrors.conf) dosyasındaki URL alias'larını okur
    fn read_url_aliases(&self) -> std::collections::HashMap<String, String> {
        use std::collections::HashMap;
        let mut aliases = HashMap::new();
        let config_paths = ["/etc/luppo/mirrors.conf", "mirrors.conf"];
    ...
    fn download_source(&self, url: &str, dest_path: &PathBuf) -> Result<(), String> {
        ...
        if let Some(stripped) = url.strip_prefix("mirrors://") {
            if let Some(slash_idx) = stripped.find('/') {
                ...
                // /etc/luppo/mirrors.conf dosyasını oku
                if let Ok(content) = std::fs::read_to_string("/etc/luppo/mirrors.conf") {
                    for line in content.lines() {
                ...
                // Python Luppo'deki gibi bilinen projeler için fallback URL'ler
                match mirror_name {
        ...
        // build.fallback (Python Luppo'deki ctx.config.values.build.fallback)
        let fallback_urls = [
            self.config.build.fallback.trim_end_matches('/'),
            "https://source.antolun.com",
        ];
    ...
    fn download_url(&self, url: &str, dest_path: &PathBuf) -> Result<(), String> {
        ...
        let client = builder
            .user_agent(concat!("luppo/", env!("CARGO_PKG_VERSION")))
            .build()
            .map_err(|e| e.to_string())?;
    ...
    fn setup_cross_compiler_env(&self, spinner: &ProgressBar) -> Result<(), String> {
        ...
        if target == "aarch64" || target == "arm64" {
            ...
            luppo_core::safe_env::set_var("CHOST", found_triple);
            luppo_core::safe_env::set_var("HOST", found_triple);
            luppo_core::safe_env::set_var("CC", format!("{}-gcc", found_triple));
            luppo_core::safe_env::set_var("CXX", format!("{}-g++", found_triple));
            luppo_core::safe_env::set_var("AR", format!("{}-ar", found_triple));
            luppo_core::safe_env::set_var("AS", format!("{}-as", found_triple));
            luppo_core::safe_env::set_var("LD", format!("{}-ld", found_triple));
            luppo_core::safe_env::set_var("RANLIB", format!("{}-ranlib", found_triple));
            luppo_core::safe_env::set_var("NM", format!("{}-nm", found_triple));
            luppo_core::safe_env::set_var("STRIP", format!("{}-strip", found_triple));

            // Cross pkg-config
            luppo_core::safe_env::set_var("PKG_CONFIG_PATH", "");
            ...
            luppo_core::safe_env::set_var("PKG_CONFIG_LIBDIR", pkg_config_libdir);
            luppo_core::safe_env::set_var("PKG_CONFIG_SYSROOT_DIR", sysroot);

            // Rust/Cargo
            luppo_core::safe_env::set_var("CARGO_BUILD_TARGET", "aarch64-unknown-linux-gnu");
            luppo_core::safe_env::set_var(
            ...
            // Rust/Cargo
            luppo_core::safe_env::set_var("CARGO_BUILD_TARGET", "x86_64-unknown-linux-gnu");
            luppo_core::safe_env::set_var(
    ...
    fn unpack_archive_with_progress(
        ...
        if ext == "zip" {
            ...
            if is_compressed_patch {
                ...
                luppo::run_command(decompress_cmd[0], &decompress_cmd[1..])
                ...
                for cmd in extract_cmds {
                    match luppo::run_command(cmd[0], &cmd[1..]) {
        ...
        if let Some(paren_start) = cmd.find('(') {
            if let Some(dot_pos) = cmd[..paren_start].rfind('.') {
                ...
                let args: Vec<String> = args_raw
                    ...
                    .filter(|a| !a.is_empty())
                    .collect();
                let idir = luppo::get::install_dir();
                let pkg_name = luppo::get::src_name();
                match module {
                    "luppotools" => Self::dispatch_luppotools(func, &args, &idir, &pkg_name, spinner),
    ...
    fn dispatch_luppotools(
        ...
        match func {
            "dodoc" => {
                for a in args {
                    luppo::dodoc(idir, pkg_name, a).map_err(|e| e.to_string())?;
                }
            }
            "doman" => {
                for a in args {
                    luppo::doman(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "dobin" => {
                for a in args {
                    luppo::dobin(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "dosbin" => {
                for a in args {
                    luppo::dosbin(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "dolib" => {
                for a in args {
                    luppo::dolib(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "dodir" => {
                for a in args {
                    luppo::dodir(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "dosym" if args.len() == 2 => {
                luppo::dosym(idir, &args[0], &args[1]).map_err(|e| e.to_string())?;
            }
            "domove" if args.len() == 2 => {
                luppo::domove(idir, &args[0], &args[1]).map_err(|e| e.to_string())?;
            }
            "remove" => {
                for a in args {
                    let full = format!(
                    ...
                    luppo::remove_path(full).map_err(|e| e.to_string())?;
                }
            }
            "dosed" if args.len() >= 2 => {
                let path = &args[0];
                ...
                luppo::dosed(path, pattern, replace, delete_line).map_err(|e| e.to_string())?;
            }
            "insinto" if args.len() >= 2 => {
                ...
                if args.len() >= 3 {
                    ...
                    luppo::insinto(idir, dest_dir, source).map_err(|e| e.to_string())?;
                }
            }
            "removeDir" => {
                for a in args {
                    luppo::remove_dir(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "installHeaders" => {
                for a in args {
                    luppo::install_headers(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "install" if args.len() == 2 => {
                luppo::install(&args[0], &args[1]).map_err(|e| e.to_string())?;
            }
            "rename" if args.len() == 2 => {
                luppo::move_path(&args[0], &args[1]).map_err(|e| e.to_string())?;
            }
            "doexe" if args.len() >= 1 => {
                let dest_dir = args.get(1).map(|s| s.as_str()).unwrap_or("");
                luppo::doexe(idir, &args[0], dest_dir).map_err(|e| e.to_string())?;
            }
            "dolib_a" if args.len() >= 1 => {
                let dest_dir = args.get(1).map(|s| s.as_str()).unwrap_or("");
                luppo::dolib_a(idir, &args[0], dest_dir).map_err(|e| e.to_string())?;
            }
            "dolib_so" if args.len() >= 1 => {
                let dest_dir = args.get(1).map(|s| s.as_str()).unwrap_or("");
                luppo::dolib_so(idir, &args[0], dest_dir).map_err(|e| e.to_string())?;
            }
            "dopixmaps" => {
                for a in args {
                    luppo::dopixmaps(idir, a).map_err(|e| e.to_string())?;
                }
            }
            "dohtml" => {
                for a in args {
                    let s: &str = a;
                    luppo::dohtml(idir, &[s], None).map_err(|e| e.to_string())?;
                }
            }
            "domo" if args.len() == 3 => {
                luppo::domo(idir, &args[0], &args[1], &args[2])
                    .map_err(|e| e.to_string())?;
            }
            "newdoc" if args.len() == 2 => {
                luppo::newdoc(idir, &args[0], &args[1]).map_err(|e| e.to_string())?;
            }
            "newman" if args.len() == 2 => {
                luppo::newman(idir, &args[0], &args[1]).map_err(|e| e.to_string())?;
            }
            other => {
                spinner.println(t!("build_unknown_luppotools", cmd = other));
            }
        }
        Ok(())
    }
    ...
    fn dispatch_kerneltools(func: &str, args: &[String], _idir: &str) -> Result<(), String> {
        match func {
            ...
            "configure" => {
                let raw_arch = luppo::get::arch().replace("i686", "i386");
                ...
                let version = luppo::get::src_version();
                let extra = Self::kernel_extra_version(&version);
                luppo::dosed("Makefile", "EXTRAVERSION =.*", &format!("EXTRAVERSION = {}", extra), false)?;
    ...
    fn kerneltools_install() -> Result<(), String> {
        let suffix = luppo::get::src_version();
        let idir = luppo::get::install_dir();
        ...
        luppo::insinto(&idir, "/boot/", "arch/x86/boot/bzImage").map_err(|e| e.to_string())?;
        ...
        for d in &["extra", "updates"] {
            luppo::dodir(&idir, &format!("/lib/modules/{}/{}", suffix, d))
                .map_err(|e| e.to_string())?;
        }
        Ok(())
    }
    ...
    fn kerneltools_install_headers() -> Result<(), String> {
        ...
        luppo::run_command("mkdir", &["-p", &dest]).map_err(|e| e.to_string())?;
        let find_cmd = format!(
        ...
        luppo::run_command(&find_cmd, &[]).map_err(|e| e.to_string())?;
        luppo::run_command("cp", &["-a", "include", "scripts", "Documentation", &dest])
            .map_err(|e| e.to_string())?;
        let _ = luppo::run_command(&format!("rm -rf {}/scripts/*.o", dest), &[]);
        let _ = luppo::run_command(&format!("rm -rf {}/scripts/*/*.o", dest), &[]);
        let _ = luppo::run_command(&format!("rm -rf {}/Documentation/DocBook", dest), &[]);
        luppo::run_command(&format!(
        ...
        luppo::dosym(&idir, &format!("/{}", hdir), &format!("/lib/modules/{}/build", suffix))
            .map_err(|e| e.to_string())?;
        luppo::dosym(&idir, "build", &format!("/lib/modules/{}/source", suffix))
            .map_err(|e| e.to_string())?;
        Ok(())
    }
    ...
    fn kerneltools_install_libc_headers(excludes: Option<Vec<String>>) -> Result<(), String> {
        ...
        let _ = luppo::run_command("rm", &["-rf", &htmp]);
        luppo::run_command("mkdir", &["-p", &htmp, &hdir]).map_err(|e| e.to_string())?;
        let raw_arch = luppo::get::arch().replace("i686", "i386");
        let kernel_arch = raw_arch.replace("x86_64", "x86");
        let o_arg = format!("O={}", htmp);
        let arch_arg = format!("ARCH={}", kernel_arch);
        let hdr_arg = format!("INSTALL_HDR_PATH={}/install", htmp);
        let work_dir = luppo::get::work_dir();
        let _ = luppo::run_command(&format!(
        ...
        let _ = luppo::run_command("rm", &["-rf", &format!("{}/sound", hdir)]);
        if let Some(exc) = excludes {
            for e in &exc {
                let _ = luppo::run_command("rm", &["-rf", &format!("{}/{}", hdir, e.trim_start_matches('/'))]);
            }
        }
        let _ = luppo::run_command("rm", &["-rf", &htmp]);
        Ok(())
        ...
        match func {
            "cd" => {
                if let Some(path) = args.first() {
                    luppo::cd(path).map_err(|e| e.to_string())?;
                }
            }
            "system" => {
                if let Some(cmd) = args.first() {
                    luppo::run_command(cmd, &[]).map_err(|e| e.to_string())?;
                }
            }
            ...
            "chown" if args.len() >= 3 => {
                let uid = &args[1];
                let gid = &args[2];
                luppo::run_command("chown", &[&format!("{}:{}", uid, gid), &args[0]])
                    .map_err(|e| e.to_string())?;
            }
            "export" if args.len() == 2 => {
                luppo_core::safe_env::set_var(&args[0], &args[1]);
            }
            "exportFlags" => luppo::export_flags(),
            "echo" if args.len() >= 2 => {
            ...
            "symlink" | "sym" if args.len() == 2 => {
                luppo::symlink(&args[0], &args[1]).map_err(|e| e.to_string())?;
            }
    ...
    fn dispatch_autotools(func: &str, args: &[String], idir: &str) -> Result<(), String> {
        ...
        match func {
            ...
            "fixInfoDir" => luppo::fix_info_dir(idir),
            "gnuconfig_update" => luppo::gnuconfig_update(),
    ...
    fn dispatch_libtools(func: &str, args: &[String], idir: &str) -> Result<(), String> {
        ...
        match func {
            "preplib" => {
                ...
                luppo::run_command("ldconfig", &["-n", "-N", dir_path.to_str().unwrap_or(dir)])
                    .map_err(|e| e.to_string())
            }
            "gnuconfig_update" => luppo::gnuconfig_update(),
            "libtoolize" => crate::actionsapi::buildtools::libtoolize(&refs),
    ...
    fn dispatch_get(func: &str, args: &[String]) -> Result<(), String> {
        match func {
            "make_jobs" | "makeJOBS" => { let _ = luppo::make_jobs(); }
            "cflags" | "CFLAGS" => { let _ = luppo::cflags(); }
            "cxxflags" | "CXXFLAGS" => { let _ = luppo::cxxflags(); }
            "ldflags" | "LDFLAGS" => { let _ = luppo::ldflags(); }
            "arch" | "ARCH" => { let _ = luppo::arch(); }
            "host" | "HOST" => { let _ = luppo::host(); }
            "cc" | "CC" => { let _ = luppo::cc(); }
            "cxx" | "CXX" => { let _ = luppo::cxx(); }
            "ar" | "AR" => { let _ = luppo::ar(); }
            "ld" | "LD" => { let _ = luppo::ld(); }
            "ranlib" | "RANLIB" => { let _ = luppo::ranlib(); }
            "nm" | "NM" => { let _ = luppo::nm(); }
            "src_name" | "srcNAME" | "srcName" => { let _ = luppo::src_name(); }
            "src_version" | "srcVERSION" | "srcVersion" => { let _ = luppo::src_version(); }
            "src_release" | "srcRELEASE" | "srcRelease" => { let _ = luppo::src_release(); }
            "install_dir" | "installDIR" | "installDir" => { let _ = luppo::install_dir(); }
            "work_dir" | "workDIR" | "workDir" => { let _ = luppo::work_dir(); }
            "src_dir" | "srcDIR" | "srcDir" => { let _ = luppo::src_dir(); }
            "src_tag" | "srcTAG" | "srcTag" => { let _ = luppo::src_tag(); }
            "pkg_dir" | "pkgDIR" | "pkgDir" => { let _ = luppo::pkg_dir(); }
            "cur_dir" | "curDIR" | "curDir" => { let _ = luppo::cur_dir(); }
            "doc_dir" | "docDIR" | "docDir" => { let _ = luppo::doc_dir(); }
            "man_dir" | "manDIR" | "manDir" => { let _ = luppo::man_dir(); }
            "info_dir" | "infoDIR" | "infoDir" => { let _ = luppo::info_dir(); }
            "data_dir" | "dataDIR" | "dataDir" => { let _ = luppo::data_dir(); }
            "conf_dir" | "confDIR" | "confDir" => { let _ = luppo::conf_dir(); }
            "libexec_dir" | "libexecDIR" | "libexecDir" => { let _ = luppo::libexec_dir(); }
            "kde_dir" | "kdeDIR" | "kdeDir" => { let _ = luppo::kde_dir(); }
            "qt_dir" | "qtDIR" | "qtDir" => { let _ = luppo::qt_dir(); }
            "cur_kernel" | "curKERNEL" | "curKernel" => { let _ = luppo::cur_kernel(); }
            "cur_python" | "curPYTHON" | "curPython" => { let _ = luppo::cur_python(); }
            "cur_perl" | "curPERL" | "curPerl" => { let _ = luppo::cur_perl(); }
            "build_type" | "buildTYPE" | "buildType" => { let _ = luppo::build_type(); }
            "emul32_prefix_dir" | "emul32_prefixDIR" => { let _ = luppo::emul32_prefix_dir(); }
            "env_var" | "ENV" => {
                if let Some(key) = args.first() {
                    let _ = luppo::env_var(key);
                }
            }
            "exist_binary" => {
                if let Some(name) = args.first() {
                    let _ = luppo::exist_binary(name);
                }
            }
            other => {
                let _ = other;
