...
fn is_emul32() -> bool {
    env::var("LUPPO_BUILD_TYPE").as_deref() == Ok("emul32")
}
...
pub fn autotools_configure(args: &[&str]) -> Result<(), String> {
    let sbindir_val = env::var("LUPPO_SBINDIR").unwrap_or_else(|_| "usr/bin".to_string());
...
fn cmake_configure_impl(args: &[&str], manage_build_dir: bool) -> Result<(), String> {
    let sbindir_val = env::var("LUPPO_SBINDIR").unwrap_or_else(|_| "bin".to_string());
...
pub fn ninja_install(dest_dir: &str, args: &[&str]) -> Result<(), String> {
    luppo_core::safe_env::set_var("DESTDIR", dest_dir);
    let jobs = super::get::make_jobs();
    ...
    luppo_core::safe_env::remove_var("DESTDIR");
    res
}
...
/// Önce dist/*.whl (pip/build ile derlenmiş) arar, bulamazsa setup.py install (orijinal Luppo) dener.
pub fn python3_setup_install(dest_dir: &str, args: &[&str]) -> Result<(), String> {
    ...
    // Hiç .whl yoksa setup.py install dene (orijinal Luppo uyumluluğu)
    let root_arg = format!("--root={}", dest_dir);
...
pub fn go_install(dest_dir: &str, args: &[&str]) -> Result<(), String> {
    ...
    luppo_core::safe_env::set_var("GOPATH", dest_dir);
    let result = run_command("go", &default_args);
    luppo_core::safe_env::remove_var("GOPATH");
    result
}
...
pub fn kernel_module_build(kernel_ver: &str, args: &[&str]) -> Result<(), String> {
    ...
    if std::env::var("SYSSRC").is_err() {
        luppo_core::safe_env::set_var("SYSSRC", &kdir);
    }
    let jobs = super::get::make_jobs();
    let mut default_args = vec![
        "-j",
