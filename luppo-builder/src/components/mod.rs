...
impl Component {
    pub fn empty(name: &str) -> Self {
        Component {
            ...
            maintainer_name: "Antolun Community".to_string(),
            maintainer_email: "admins@antolun.com".to_string(),
        }
    }
}
...
fn default_mname() -> String {
    "Antolun Community".to_string()
}
fn default_memail() -> String {
    "admins@antolun.com".to_string()
}
...
impl XmlComponents {
    ...
    fn write(&self, components: &[Component]) -> Result<(), String> {
        ...
        let mut content = String::from("<LUPPO>\n    <Components>\n");
        for comp in &sorted {
...
impl KdlComponents {
    ...
    fn write(&self, components: &[Component]) -> Result<(), String> {
        ...
        let mut content = String::from("// LupuS Components\n");
        for comp in &sorted {
...
fn find_component_dirs(
    ...
    if !rel_parts.contains(&"files") && !rel_parts.contains(&"comar") {
        ...
        if !comp_name.is_empty() {
            if !has_component && !has_lopec {
                if has_actions {
                    println!("WARNING: {}/lopec.xml not exists", dir.display());
                } else {
                    let mut has_inner_lopec = false;
                    check_inner_lopec(dir, &mut has_inner_lopec).map_err(|e| e.to_string())?;
                    if has_inner_lopec {
                        if fix {
...
fn check_inner_lopec(dir: &Path, found: &mut bool) -> std::io::Result<()> {
    if *found {
        return Ok(());
    }
    for entry in fs::read_dir(dir)? {
        ...
        let path = entry.path();
        if path.is_dir() {
            check_inner_lopec(&path, found)?;
        } else if path.file_name().and_then(|n| n.to_str()) == Some("lopec.xml") {
            *found = true;
            return Ok(());
        }
    }
