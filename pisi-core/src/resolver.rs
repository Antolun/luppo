...
use crate::version::PisiVersion;
use crate::{PisiError, PisiResult};
...
use pisi_spec::models::{Dependencies, PackageActions, PackageDefinition, Packager};
        ...
        if let Ok(all_packages) = db.list_available_packages() {
            for pkg in all_packages {
                ...
                let pkg_def = PackageDefinition {
                    ...
                    packager: Packager {
                        name: "Pisi Community".to_string(),
                        email: "info@pisilinux.org".to_string(),
                    },
                    deps: Dependencies {
...
pub struct PackageResolver {
    ...
    pubgrub_provider: Option<OfflineDependencyProvider<String, Ranges<PisiVersion>>>,
}

impl PackageResolver {
    pub fn new(db: PisiDatabase, repo: PisiRepo) -> Self {
        PackageResolver {
            db,
            repo,
    ...
    fn get_pubgrub_provider(
        &mut self,
    ) -> PisiResult<&OfflineDependencyProvider<String, Ranges<PisiVersion>>> {
    ...
    fn build_pubgrub_provider(
        ...
        for (name, pkg_def) in &self.repo.packages {
            let version = PisiVersion::new(&pkg_def.version);
    ...
    pub fn resolve_deps(&mut self, package_names: &[String]) -> PisiResult<Vec<PackageDefinition>> {
        // This is for runtime
        ...
        let root_version = PisiVersion::new("1.0.0");
            ...
            Ok(solution) => {
                ...
                match toposort(&graph, None) {
                    ...
                    Err(cycle) => {
                        ...
                        Err(PisiError::CycleDependency(visual_graph))
                    }
                }
            }
            Err(pubgrub_err) => {
                ...
                match pubgrub_err {
                    pubgrub::PubGrubError::NoSolution(tree) => {
                        ...
                        Err(PisiError::RuntimeError(format!(
                            "PubGrub Resolution Error:\n{}",
                            report
                        )))
                    }
                    err => Err(PisiError::RuntimeError(format!("PubGrub Error: {:?}", err))),
                }
            }
        }
    }
    ...
    /// Basit DFS + visited set yaklaşımı (Python pisi benzeri) - çok daha hızlı.
    /// `PisiRepo` kullanmaz, doğrudan DB üzerinden tekil paket sorguları yapar (tüm paketleri yüklemez).
    pub fn resolve_build_deps(
        &mut self,
        package_names: &[String],
        build_env_installed: &HashSet<String>,
    ) -> PisiResult<Vec<PackageDefinition>> {
    ...
    pub fn check_reverse_deps(&self, package_name: &str) -> PisiResult<Vec<String>> {
    ...
    pub fn check_transitive_reverse_deps(&self, package_name: &str) -> PisiResult<Vec<String>> {
        let (graph, nodes) = self.build_installed_graph()?;
    ...
    fn build_installed_graph(
        &self,
    ) -> PisiResult<(DiGraph<String, ()>, HashMap<String, NodeIndex>)> {
...
/// Package (pisi-core) -> PackageDefinition (pisi-spec) dönüşümü
...
/// PackageResolver::resolve_build_deps ve build.rs tarafından kullanılır (PisiRepo üzerinden).
    ...
    while let Some((current, processed_children)) = process_stack.pop() {
        ...
        if visited.contains(&current) || in_stack.contains(&current) {
            if in_stack.contains(&current) {
                return Err(PisiError::CycleDependency(format!(
...
/// Free function: `PackageResolver` veya `PisiRepo` gerektirmez, doğrudan DB kullanır.
...
mod tests {
    ...
    fn create_mock_pkg(
        ...
        PackageDefinition {
            ...
            license: "GPL".to_string(),
            packager: Packager {
                name: "Pisi Test".to_string(),
                email: "test@pisilinux.org".to_string(),
            },
            deps: Dependencies {
            ...
            runtime_dependencies: Some(pisi_spec::models::RuntimeDeps {
                dependencies: runtime_deps
                    .iter()
                    .map(|&s| pisi_spec::models::Dependency {
                        name: s.to_string(),
                        ..Default::default()
                    })
                    .collect(),
    ...
    fn test_simple_circular_dependency() {
        ...
        let db = PisiDatabase::open(dir.path().to_path_buf()).expect("DB açılamadı");
        ...
        let repo = PisiRepo { packages: repo_map };
        ...
        if let Err(PisiError::CycleDependency(visual)) = result {
            assert!(
