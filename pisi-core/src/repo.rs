...
use crate::package::{
    Component, ComponentsRoot, FileMetadata, FilesXmlRoot, InstalledPackage, Package, PisiIndex,
    PisiRoot,
};
use crate::resolver::{PackageResolver, PisiRepo};
use crate::PisiError;
use chrono::Local;
use indicatif::{ProgressBar, ProgressStyle};
use pisi_spec::models::HistoryAction;
use rust_i18n::t;
...
impl Repository {
    pub fn new(db: PisiDatabase, config: crate::config::Config) -> Self {
        Repository { db, config }
    }

    fn get_client(&self) -> PisiResult<reqwest::blocking::Client> {
        ...
        if let Some(ref proxy) = self.config.general.http_proxy {
            builder = builder.proxy(
                reqwest::Proxy::http(proxy).map_err(|e| PisiError::RuntimeError(e.to_string()))?,
            );
        }
        if let Some(ref proxy) = self.config.general.https_proxy {
            builder = builder.proxy(
                reqwest::Proxy::https(proxy).map_err(|e| PisiError::RuntimeError(e.to_string()))?,
            );
        }
    ...
    pub fn add_repo(&self, name: &str, url: &str) -> PisiResult<()> {
    ...
    pub fn remove_repo(&self, name: &str) -> PisiResult<()> {
        if !self.db.remove_repo(name)? {
            return Err(PisiError::RuntimeError(format!(
                "{}: {}",
    ...
    pub fn set_repo_status(&self, name: &str, enabled: bool) -> PisiResult<()> {
    ...
    fn update_repositories_internal(&self, _trace_id: u64) -> PisiResult<()> {
        ...
        for repo in active_repos {
            ...
            // Yerel önbellek (/var/lib/pisi/index/<repo_adı>/) — ağ yoksa veya download başarısızsa
            if fetched.is_none() {
            ...
            if let Some(pisi_index) = fetched {
                let total_count = pisi_index.packages.len() + pisi_index.spec_files.len();
                println!("{}", t!("repo_xml_processing", count = total_count));
                self.process_pisi_index(pisi_index, &repo.url, &repo.name)?;
    ...
    /// Paketi önbelleğini (/var/cache/pisi/packages) temizler.
    pub fn perform_delete_cache(&self) -> PisiResult<()> {
        let cache_dir = &self.config.directories.cached_packages_dir;
        println!("{}", t!("repo_cache_cleaning", path = cache_dir.display()));
        if cache_dir.exists() {
            let entries = fs::read_dir(cache_dir).map_err(PisiError::IoError)?;
            let mut count = 0;

            for entry in entries {
                let entry = entry.map_err(PisiError::IoError)?;
                if entry.path().is_file() {
                    fs::remove_file(entry.path()).map_err(PisiError::IoError)?;
                    count += 1;
                }
            }
    ...
    fn select_best_mirror(&self, mirrors: &[String], auth: &Option<(String, String)>) -> Option<String> {
        ...
        for mirror in mirrors {
            let test_url = mirror.trim_end_matches('/').to_string() + "/pisi-index.json.xz";
    ...
    pub fn fetch_package(
        ...
        if !target_dir.exists() {
            fs::create_dir_all(&target_dir).map_err(PisiError::IoError)?;
        }
        ...
        // Arşiv adını oluştur (ncurses-6.5-1-p2-x86_64.pisi gibi)
        let archive_name = format!("{}-{}-{}-p2-x86_64.pisi", pkg.name, version, release_no);

        let base_url = if !pkg.repo_url.is_empty() {
            pkg.repo_url.trim_end_matches('/').to_string()
        } else {
            "https://stable2.pisilinux.org".to_string()
        };
        ...
        // --- 2. ÖNBELLEK KONTROLÜ ---
        if dest_path.exists() {
            let file = fs::File::open(&dest_path).map_err(PisiError::IoError)?;
            if zip::ZipArchive::new(file).is_err() {
                println!("{}", t!("repo_cache_corrupted"));
                fs::remove_file(&dest_path).map_err(PisiError::IoError)?;
            } else {
        ...
        let mut response = req_builder
            .send()
            .map_err(|e| PisiError::RuntimeError(t!("repo_error_connection", error = e).into()))?;
        ...
        let mut file = fs::File::create(&dest_path).map_err(PisiError::IoError)?;
        ...
        while let Ok(n) = response.read(&mut buffer) {
            ...
            let chunk = &buffer[..n];
            file.write_all(chunk).map_err(PisiError::IoError)?;
        ...
        let sig_url = final_url.replace(".pisi", ".sig");
    ...
    fn resolve_index_url(base_url: &str, filename: &str) -> String {
        ...
        // URL başka bir index dosyasına işaret ediyorsa (örn. pisi-index.json.xz),
        // onu temizleyip yeni dosya adını ekle.
        if let Some(base_pos) = base.rfind("/pisi-index.") {
    ...
    fn try_fetch_json_xz(&self, base_url: &str) -> PisiResult<PisiIndex> {
        let url = Self::resolve_index_url(base_url, "pisi-index.json.xz");
        ...
        let response = client.get(&url).send().map_err(|e| {
            PisiError::RuntimeError(t!("repo_error_download", error = e).to_string())
        })?;
        ...
        decoder
            ...
            .map_err(|e| PisiError::RuntimeError(t!("error_xz_extract", error = e).to_string()))?;

        let pisi_root: PisiIndex = serde_json::from_str(&json_content)
            .map_err(|e| PisiError::RuntimeError(t!("repo_error_json", error = e).to_string()))?;

        Ok(pisi_root)
    }

    /// XML.xz indeksi çek ve çöz
    fn try_fetch_legacy_xml_xz(&self, base_url: &str) -> PisiResult<PisiIndex> {
        let url = Self::resolve_index_url(base_url, "pisi-index.xml.xz");
        ...
        // 2. XML'i PisiIndex yapısına parse et
        // <PISI> etiketini karşılamak için PisiIndex kullanıyoruz
        let pisi_root: PisiIndex = serde_xml_rs::from_str(&xml_content)
            .map_err(|e| PisiError::RuntimeError(t!("error_xml_parse", error = e).to_string()))?;

        Ok(pisi_root)
    }

    /// Klasik components.xml.xz Çekme ve Çözme
    fn try_fetch_components_xml_xz(&self, base_url: &str) -> PisiResult<Vec<Component>> {
        ...
        let mut response = client
            .get(&url)
            .send()
            .map_err(|e| PisiError::RuntimeError(t!("repo_error_download", error = e).into()))?;
        ...
        let pisi_root: ComponentsRoot = serde_xml_rs::from_str(&xml_content)
            .map_err(|e| PisiError::RuntimeError(t!("error_xml_parse", error = e).to_string()))?;

        Ok(pisi_root.components.items)
    }

    /// Yerel indeks önbelleğinden (.xml.xz) okumayı dener.
    /// Eski pisi, indeks dosyalarını /var/lib/pisi/index/<repo_adı>/pisi-index.xml.xz yolunda önbelleğe alır.
    fn try_load_local_index(&self, repo_name: &str) -> PisiResult<PisiIndex> {
        let cache_path = self.config.directories.index_dir.join(repo_name).join("pisi-index.xml.xz");
        ...
        let xz_data = fs::read(&cache_path).map_err(PisiError::IoError)?;
        let mut decoder = XzDecoder::new(&xz_data[..]);
        let mut xml_content = String::new();
    ...
    fn process_pisi_index(
        ...
        for spec in index.spec_files {
            if let Some(ref source_uri) = spec.source.source_uri {
                // Uzak pspec.xml URL'si: <repo_url>/<source_uri>
    ...
    pub fn perform_add_repo(&self, name: &str, url: &str, trace_id: u64) -> PisiResult<()> {
        println!("{}", t!("repo_syncing", name = name, url = url));

        let mut pisi_index = None;
        let mut components = None;

        // SIKIŞTIRILMIŞ JSON Denemesi (.json.xz)
        match self.try_fetch_json_xz(url) {
            Ok(fetched_pisi_index) => {
                println!("{}", t!("repo_json_success"));
                let total_count = fetched_pisi_index.packages.len() + fetched_pisi_index.spec_files.len();
                println!("{}", t!("repo_xml_processing", count = total_count));
                pisi_index = Some(fetched_pisi_index);
        ...
        // Save index to DB
        if let Some(idx) = pisi_index {
            self.process_pisi_index(idx, url, name)?;
    ...
    pub fn perform_remove_repo(&self, name: &str, trace_id: u64) -> PisiResult<()> {
    ...
    pub fn perform_list_repos(&self) -> PisiResult<()> {
        ...
        for repo in repos {
                ...
                || repo.url.ends_with(".xz")
            {
                ...
                format!("{}/pisi-index.json.xz", repo.url.trim_end_matches('/'))
            };
            println!("   {}", index_url);
        }
        Ok(())
    ...
    pub fn perform_fetch(
        ...
        if include_deps {
            ...
            let repo = PisiRepo::new(self.db.clone());
    ...
    pub fn perform_rebuild_db(&self, trace_id: u64) -> PisiResult<()> {
        ...
        // Daima /var/lib/pisi/package dizininden kurulu paketleri senkronize et
    ...
    /// /var/lib/pisi/package dizinini tarayarak kurulu paketleri DB'ye geri yükler.
    pub fn rebuild_installed_db(&self) -> PisiResult<()> {
    ...
    ) -> PisiResult<InstalledPackage> {
        let metadata_content = fs::read_to_string(metadata_path).map_err(PisiError::IoError)?;
        let pisi_root: PisiRoot = serde_xml_rs::from_str(&metadata_content).map_err(|e| {
            PisiError::RuntimeError(t!("error_metadata_parse", error = e).to_string())
        })?;

        let mut pkg = pisi_root.package;
        if let Some(latest) = pkg.history.updates.first() {
            pkg.version = latest.version.clone();
            pkg.release = latest.release;
        }

        let files_content = fs::read_to_string(files_path).map_err(PisiError::IoError)?;
        let files_root: FilesXmlRoot = serde_xml_rs::from_str(&files_content)
            .map_err(|e| PisiError::RuntimeError(t!("error_files_parse", error = e).to_string()))?;
    ...
    pub fn perform_clean(&self) -> PisiResult<()> {
    ...
    pub fn perform_update(&self, trace_id: u64) -> PisiResult<()> {
    ...
    pub fn find_updates(
        ...
        db: &PisiDatabase,
        compare_hashes: bool,
    ) -> PisiResult<Vec<(String, String, String)>> {
    ...
    /// Uzak bir URL'den pspec dosyasını ve onunla ilişkili companion (actions.py, translations.xml vb.),
    /// ek dosyaları (additional_files) ve yamaları (patches) indirip yerel bir geçici dizine kaydeder.
    pub fn download_remote_spec(&self, url: &str) -> PisiResult<PathBuf> {
        let base_url = if url.ends_with("/pspec.xml") {
            url.trim_end_matches("pspec.xml")
        } else if url.ends_with("/pspec.kdl") {
            url.trim_end_matches("pspec.kdl")
        } else {
            return Err(PisiError::RuntimeError(
                t!("repo_err_invalid_pspec_url").to_string(),
            ));
        };

        // Benzersiz geçici dizin oluştur
        let unique_id = chrono::Utc::now().timestamp_millis();
        let temp_dir = PathBuf::from(format!("/var/tmp/pisi/remote-build/{}", unique_id));
        fs::create_dir_all(&temp_dir).map_err(PisiError::IoError)?;

        let filename = url.split('/').next_back().unwrap_or("pspec.xml");
        let pspec_file_path = temp_dir.join(filename);
        ...
        let res = client
            ...
            .map_err(|e| PisiError::RuntimeError(e.to_string()))?;
        if !res.status().is_success() {
            return Err(PisiError::RuntimeError(t!(
                "repo_err_pspec_download_failed",
        ...
        let content = res
            .text()
            .map_err(|e| PisiError::RuntimeError(e.to_string()))?;
        fs::write(&pspec_file_path, &content).map_err(PisiError::IoError)?;

        // pspec'i parse et ki bağımlılıklarını/yamalarını/ek dosyalarını bilelim
        let spec = pisi_spec::models::PisiSpec::from_path(&pspec_file_path).map_err(|e| {
            PisiError::RuntimeError(t!("repo_err_pspec_parse_failed", error = e).to_string())
        })?;
        ...
        let ensure_files_dir = |created: &mut bool, dir: &PathBuf| -> Result<(), PisiError> {
            if !*created {
                fs::create_dir_all(dir).map_err(PisiError::IoError)?;
                *created = true;
            }
            Ok(())
        };
    ...
    pub fn perform_check_repo(&self, check_circular: bool) -> PisiResult<()> {
    ...
    fn fetch_standalone_index(&self, uri: &str) -> PisiResult<PisiIndex> {
        ...
        if uri.starts_with("http://") || uri.starts_with("https://") {
            ...
            res.read_to_end(&mut raw_data).map_err(PisiError::IoError)?;
        } else {
            let path = uri.strip_prefix("file://").unwrap_or(uri);
            let mut file = fs::File::open(path).map_err(PisiError::IoError)?;
        ...
        let content_str = if is_xz {
            ...
            String::from_utf8(raw_data).map_err(|e| PisiError::RuntimeError(e.to_string()))?
        };
    ...
    pub fn perform_repo_diff(&self, source_index: &str, binary_index: &str) -> PisiResult<()> {
        println!(
            "{}",
