...
use pisi_spec::models::TriggersConfig;
use rust_i18n::t;
...
pub trait ComarBackend {
    fn register_package(&self, pkg_name: &str, model: &str, script_path: &str) -> PisiResult<()>;
    fn remove_package(&self, pkg_name: &str) -> PisiResult<()>;
    fn register_service_state(&self, app_name: &str) -> PisiResult<()>;
    fn run_package_script(&self, pkg_name: &str, action: ComarAction) -> PisiResult<()>;
}
...
impl ComarBackend for DBusComarBackend {
    fn register_package(&self, pkg_name: &str, model: &str, script_path: &str) -> PisiResult<()> {
        let conn = Connection::system().map_err(|e| {
            PisiError::RuntimeError(t!("comar_err_dbus_conn", error = e).to_string())
        })?;
        ...
        conn.call_method(
            ...
            .map_err(|e| {
                PisiError::RuntimeError(t!("comar_err_register", error = e).to_string())
            })?;
    ...
    fn remove_package(&self, pkg_name: &str) -> PisiResult<()> {
        ...
        conn.call_method(Some(dest), path, Some(interface), "remove", &(pkg_name,))
            .map_err(|e| PisiError::RuntimeError(t!("comar_err_remove", error = e).to_string()))?;
    ...
    fn register_service_state(&self, app_name: &str) -> PisiResult<()> {
        ...
        conn.call_method(Some(dest), path.as_str(), Some(iface), "registerState", &())
            .map_err(|e| {
                PisiError::RuntimeError(t!("comar_err_state", error = e).to_string())
            })?;
...
impl SubprocessComarBackend {
    ...
    pub fn load_system_triggers(&self, dir: &Path) -> PisiResult<Vec<SystemTrigger>> {
...
impl ComarBackend for SubprocessComarBackend {
    ...
    fn remove_package(&self, _pkg_name: &str) -> PisiResult<()> {
        Ok(())
    }
    ...
    fn run_package_script(&self, pkg_name: &str, action: ComarAction) -> PisiResult<()> {
        ...
        let status = cmd.status().map_err(PisiError::IoError)?;
    ...
    fn register_service_state(&self, _app_name: &str) -> PisiResult<()> {
        Ok(())
    }
...
impl ComarManager {
    ...
    pub fn remove_package(&self, pkg_name: &str) -> PisiResult<()> {
        self.backend.remove_package(pkg_name)
    }

    pub fn register_service_state(&self, app_name: &str) -> PisiResult<()> {
        self.backend.register_service_state(app_name)
    }

    pub fn run_package_script(&self, pkg_name: &str, action: ComarAction) -> PisiResult<()> {
        self.backend.run_package_script(pkg_name, action)
    }

    pub fn run_system_triggers(&self, affected_files: &[String]) -> PisiResult<()> {
...
fn load_system_triggers(trigger_dir: &Path) -> PisiResult<Vec<SystemTrigger>> {
    let triggers_file = trigger_dir.join("triggers.xml");
    if !triggers_file.exists() {
        return Ok(Vec::new());
    }
    let content = fs::read_to_string(&triggers_file).map_err(PisiError::IoError)?;
    let root: TriggersConfig = quick_xml::de::from_str(&content)
        .map_err(|e| PisiError::RuntimeError(format!("Tetikleyici dosyası ayrıştırılamadı: {e}")))?;
