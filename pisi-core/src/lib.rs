...
pub use error::PisiError;

use database::PisiDatabase;
use std::path::Path;

// PisiResult'i tüm modüller için kökte tanımlıyoruz.
pub type PisiResult<T> = Result<T, PisiError>;

use rust_i18n::t;
// Pisi çekirdek kütüphanesi başlatıcısı.
pub fn initialize_pisi_core<P: AsRef<Path>>(db_path: P) -> Result<PisiDatabase, String> {
    ...
    match PisiDatabase::open(path_buf) {
        // <<< PathBuf kullanıldı
        Ok(db) => Ok(db),
