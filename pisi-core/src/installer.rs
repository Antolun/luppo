...
use crate::package::{FileMetadata, FileXmlEntry, FilesXmlRoot, InstalledPackage, Package, PisiRoot, SourceInfo};
...
impl Installer {
    ...
    pub fn backup_db(&self, backup_dir: &PathBuf) -> PisiResult<()> {
        self.db.backup_to_dir(backup_dir)
    }

    /// Veritabanını yedekten geri yükle
    pub fn restore_db(&self, backup_dir: &PathBuf) -> PisiResult<()> {
        self.db.restore_from_dir(backup_dir)
    }

    /// Veritabanı bütünlüğünü kontrol et
    pub fn verify_db(&self) -> PisiResult<()> {
    ...
    fn restorecon_paths(&self, paths: &[String]) -> PisiResult<()> {
        ...
        for rel_path in paths {
            ...
            if full_path.exists() {
                let status = Command::new("restorecon")
                    ...
                    .status()
                    .map_err(|e| PisiError::RuntimeError(format!("restorecon failed: {}", e)))?;
                if !status.success() {
                    eprintln!(
                        "{}",
                        t!(
    ...
    fn create_users(
        &self,
        users: &pisi_spec::models::UsersWrapper,
    ) -> PisiResult<()> {
        for user in &users.users {
            ...
            let status = cmd.status().map_err(|e| PisiError::RuntimeError(e.to_string()))?;
            if !status.success() {
    ...
    fn create_groups(
        &self,
        groups: &pisi_spec::models::GroupsWrapper,
    ) -> PisiResult<()> {
    ...
    pub fn install_packages(&self, packages: &[String]) -> PisiResult<()> {
    ...
    pub fn check_file_conflicts(&self, pkg_name: &str, files: &HashSet<String>) -> PisiResult<()> {
    ...
    /// Pisi dosyalarından gerçek kurulum yapan ana fonksiyon
    ...
    pub fn run_pre_install(&self, pkg_name: &str, script: &str, rootfs: &str) -> PisiResult<()> {
        println!("{}", t!("script_pre_install", package = pkg_name));

        let status = std::process::Command::new("sh")
            .arg("-c")
            .arg(script)
            .env("PISI_DESTDIR", rootfs)
            .status()
            .map_err(|e| PisiError::RuntimeError(t!("error_script_run", error = e).to_string()))?;
    ...
    pub fn run_post_install(&self, pkg_name: &str, script: &str, rootfs: &str) -> PisiResult<()> {
    ...
    pub fn run_pre_upgrade(&self, pkg_name: &str, script: &str, rootfs: &str) -> PisiResult<()> {
    ...
    pub fn run_post_upgrade(&self, pkg_name: &str, script: &str, rootfs: &str) -> PisiResult<()> {
    ...
    pub fn run_pre_remove(&self, pkg_name: &str, script: &str, rootfs: &str) -> PisiResult<()> {
    ...
    pub fn run_post_remove(&self, pkg_name: &str, script: &str, rootfs: &str) -> PisiResult<()> {
    ...
    fn remove_package_files(&self, pkg: &InstalledPackage) -> PisiResult<()> {
        println!(
            "{}",
            t!(
    ...
    pub fn calculate_remove_chain(&self, target_pkg: &str) -> PisiResult<Vec<String>> {
    ...
    pub fn get_reverse_dependencies(&self, package_name: &str) -> PisiResult<Vec<String>> {
    ...
    pub fn find_orphaned_packages(&self) -> PisiResult<Vec<String>> {
    ...
    pub fn perform_install(
        ...
        let mut local_pisi_paths: HashMap<String, PathBuf> = HashMap::new();
        let mut local_packages: HashMap<String, crate::package::Package> = HashMap::new();
        let mut resolved_package_queries = Vec::new();

        let mut repo = PisiRepo::new(self.db.clone());

        for name in &package_names {
            let path = std::path::Path::new(name);
            if name.ends_with(".pisi") || path.is_file() {
                let mut pkg_data = Packager::read_package(name)?;

                // Calculate the SHA1 hash of the local .pisi file itself for the integrity check
                let mut file = fs::File::open(path).map_err(PisiError::IoError)?;
                let mut hasher = Sha1::new();
                io::copy(&mut file, &mut hasher).map_err(PisiError::IoError)?;
                let pisi_file_sha1 = format!("{:x}", hasher.finalize());
                pkg_data.metadata.package_hash = pisi_file_sha1;
                ...
                let pkg_def = PackageDefinition {
                    ...
                    packager: pisi_spec::models::Packager {
                        name: pkg_data
                            .metadata
                            .history
                            .updates
                            .first()
                            .map(|u| u.name.clone())
                            .unwrap_or_else(|| "Pisi Community".to_string()),
                        email: pkg_data
                            .metadata
                            .history
                            .updates
                            ...
                            .unwrap_or_else(|| "info@pisilinux.org".to_string()),
                    },
                    deps: pisi_spec::models::Dependencies {
                    ...
                    actions: pisi_spec::models::PackageActions {
                        steps: Vec::new(),
                        step_types: Vec::new(),
                        configure: None,
                        pre_install: None,
                    ...
                    runtime_dependencies: Some(pisi_spec::models::RuntimeDeps {
                        dependencies: runtime_deps_list
                            .into_iter()
                            .map(|d| pisi_spec::models::Dependency {
                                name: d,
                                ..Default::default()
                            })
                            .collect(),
                ...
                local_pisi_paths.insert(pkg_def.name.clone(), PathBuf::from(name));
        ...
        loop {
            match resolver.resolve_deps(&resolved_package_queries) {
                ...
                Err(PisiError::InstalledConflict {
                    package,
                    conflicting_package,
                }) => {
                    println!(
        ...
        for pkg_def in &installation_plan {
            if let Some(local_path) = local_pisi_paths.get(&pkg_def.name) {
        ...
        if !ignore_check {
            ...
            for (_, full_pkg_data, pisi_path) in &downloaded_packages {
                let mut file = fs::File::open(pisi_path).map_err(PisiError::IoError)?;
                let mut hasher = Sha1::new();
                io::copy(&mut file, &mut hasher).map_err(PisiError::IoError)?;
        ...
        for (index, (pkg_def, full_pkg_data, pisi_path)) in
            downloaded_packages.into_iter().enumerate()
        {
            ...
            // --- GPG İMZA DOĞRULAMA ---
            let signature_verified = self.verify_package_signature(&pisi_path)?;

            let is_delta_pkg = pisi_path
                .to_str()
                .map(|s| s.contains(".delta.pisi"))
                .unwrap_or(false);

            let mut package_data = Packager::read_package(pisi_path.to_str().unwrap())?;
            ...
            // --- DELTA UYGULAMA ---
            // Eğer bu bir .delta.pisi ise, sadece değişen dosyaları içerir.
            ...
            // COMAR scriptlerini .pisi zip'inden çıkar (install.tar.xz dışında ayrı ZIP entry)
            if let Ok(comar_zip) = std::fs::File::open(&pisi_path) {
                if let Ok(mut archive) = ZipArchive::new(comar_zip) {
                    for i in 0..archive.len() {
                        if let Ok(mut entry) = archive.by_index(i) {
                            ...
                            let target = self.dest_dir().join("var/lib/pisi/package")
            ...
            // Python PiSi uyumluluğu için filesystem'e metadata.xml ve files.xml yaz
            let pkg_dir = self.config.directories.packages_dir
            ...
            if let Ok(pkg_dir) = std::fs::canonicalize(&pkg_dir) {
                ...
                let mut buffer = String::new();
                {
                    ...
                    serializer.indent(' ', 2);
                    use serde::Serialize;
                    let _ = pisi_root.serialize(serializer);
                }
    ...
    pub fn perform_configure_pending(&self) -> PisiResult<()> {
        let installed = self.db.list_installed_packages()?;
    ...
    pub fn perform_remove_orphaned(&self, trace_id: u64) -> PisiResult<()> {
    ...
    /// Not: Bu fonksiyon sistemde 'gpgv' komutunun ve Pisi anahtarlarının yüklü olmasını bekler.
    fn verify_package_signature(&self, path: &std::path::Path) -> PisiResult<bool> {
        ...
        let status = std::process::Command::new("gpgv")
            .arg("--keyring")
            .arg("/etc/pisi/trusted-keys.gpg")
            .arg(&sig_path)
            .arg(path)
            .status()
            .map_err(|e| {
                PisiError::RuntimeError(format!("GPG doğrulaması başlatılamadı: {}", e))
            })?;
    ...
    pub fn perform_rollback(&self, target_trace_id: u64, current_trace_id: u64) -> PisiResult<()> {
...
mod tests {
    ...
    fn test_installer_safety_switch() {
        ...
        let db = PisiDatabase::open(dir.path().to_path_buf()).expect("Failed to open DB");
        ...
        if let Err(PisiError::RuntimeError(err_msg)) = result {
