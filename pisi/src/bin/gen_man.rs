...
#[command(
    author,
    version,
    about = "Rust-based Pisi Package Manager",
    long_about = None,
...
fn main() {
    ...
    let mut cmd2 = PisiCli::command();
    for shell in [Shell::Bash, Shell::Zsh, Shell::Fish, Shell::PowerShell] {
        let mut buf: Vec<u8> = Vec::new();
        clap_complete::generate(shell, &mut cmd2, "pisi", &mut buf);
        let ext = match shell {
            Shell::Bash => "bash",
            Shell::Zsh => "zsh",
            Shell::Fish => "fish",
        ...
        let comp_path = out_dir.join(format!("pisi.{}", ext));
