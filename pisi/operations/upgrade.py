...
def upgrade_base(A = set()):
    ...
    if not ctx.config.values.general.ignore_safety and not ctx.get_option('ignore_safety'):
        if componentdb.has_component('system.base'):
            ...
            extra_installs = list(filter(lambda x: not installdb.has_package(x), systembase - set(A)))
            ...
            extra_upgrades = list(filter(lambda x: is_upgradable(x), systembase - set(install_order)))
            upgrade_order = []
