...
class ComponentDB(lazydb.LazyDB):
    ...
    def search_component(self, terms, lang=None, repo=None):
        ...
        for name, xml in self.cdb.get_items_iter(repo):
            if name not in found and terms == list(filter(lambda term: re.compile(rename % (lang, term), re.I).search(xml) or \
    ...
    def get_packages(self, component_name, repo=None, walk=False):
        ...
        sub_components = list(filter(lambda x:x.startswith(component_name+"."), self.list_components(repo)))
    ...
    def get_union_packages(self, component_name, walk=False):
        ...
        sub_components = list(filter(lambda x:x.startswith(component_name+"."), self.list_components()))
        for sub in sub_components:
            try:
