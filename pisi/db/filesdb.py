...
class FilesDB(lazydb.LazyDB):
    ...
    def has_file(self, path):
        return self.__hash(path) in self.filesdb

    def get_file(self, path):
        return self.filesdb[self.__hash(path)], path
    ...
    def add_files(self, pkg, files):
        ...
        for f in files.list:
            self.filesdb[self.__hash(f.path)] = pkg

    def remove_files(self, files):
        for f in files:
            h = self.__hash(f.path)
            if h in self.filesdb:
                del self.filesdb[h]
