...
class File:
    ...
    def decompress(localfile, compress):
        ...
        if compress == File.COMPRESSION_TYPE_XZ:
            import lzma
            with lzma.open(localfile, "rb") as f_in:
                with open(localfile[:-3], "wb") as f_out:
                    f_out.write(f_in.read())
            localfile = localfile[:-3]
        elif compress == File.COMPRESSION_TYPE_BZ2:
            import bz2
            with bz2.open(localfile, "rb") as f_in:
                with open(localfile[:-4], "wb") as f_out:
                    f_out.write(f_in.read())
    ...
    def download(uri, transfer_dir = "/tmp", sha1sum = False,
        ...
        if sha1sum:
            ...
            with open(sha1filename, "r") as sha1f:
    ...
    def close(self, delete_transfer = False):
        ...
        if self.mode == File.write:
            ...
            if ctypes & File.COMPRESSION_TYPE_XZ:
                ...
                with open(self.localfile, "rb") as f_in:
                    with lzma.open(compressed_file, "wb", preset=9) as f_out:
                        f_out.write(f_in.read())
