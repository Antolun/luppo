...
pub struct LuppoVersion(pub String);

impl LuppoVersion {
    pub fn new(v: &str) -> Self {
        LuppoVersion(v.to_string())
    }
}

impl Display for LuppoVersion {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", self.0)
    }
}

impl Debug for LuppoVersion {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "\"{}\"", self.0)
    }
}

impl Ord for LuppoVersion {
...
impl PartialOrd for LuppoVersion {
