use crate::error::LuppoResult;
use crate::package::{FileData, FilesXmlRoot, Package, LuppoPackageData, LuppoRoot};
...
impl Packager {
    /// Bir .luppo (ZIP) veya .deb (ar) paketini okur, metadata ve dosyaları ayrıştırır.
    pub fn read_package(path: &str) -> LuppoResult<LuppoPackageData> {
        if path.ends_with(".deb") {
            return Self::read_deb_package(path);
        }

        let mut file = std::fs::File::open(path).map_err(LuppoError::IoError)?;
        let mut magic = [0u8; 8];
        ...
        let file = std::fs::File::open(path).map_err(LuppoError::IoError)?;

        // ZIP Arşivini aç (Luppo paketleri PK.. imzalı birer ZIP'tir)
        let mut zip_archive = ZipArchive::new(file).map_err(|e| {
            LuppoError::RuntimeError(t!("packager_error_zip", error = e).to_string())
        })?;
        ...
        for i in 0..zip_archive.len() {
            let mut zip_file = zip_archive
                .by_index(i)
                .map_err(|e| LuppoError::IoError(std::io::Error::other(e)))?;
            ...
            if name.ends_with("metadata.xml") {
                ...
                // XML -> LuppoRoot (LUPPO etiketi) -> Package
                let root: LuppoRoot = quick_xml::de::from_str(&content).map_err(|e| {
            ...
            // 3. install.tar.xz Okuma (Gerçek sistem dosyaları)
                ...
                for tar_entry_result in tar_archive.entries().map_err(LuppoError::IoError)? {
                    let mut tar_entry: TarEntry<_> =
                        tar_entry_result.map_err(LuppoError::IoError)?;
        ...
        let metadata = metadata
            .ok_or_else(|| LuppoError::RuntimeError(t!("packager_error_no_metadata").to_string()))?;
    ...
    /// Belirtilen dizindeki dosyaları .luppo paketine dönüştürür.
    pub fn create_package(
        mut metadata: Package,
        source_dir: &std::path::Path,
        output_path: &std::path::Path,
    ) -> LuppoResult<()> {
        let file = File::create(output_path).map_err(LuppoError::IoError)?;
        ...
        let mut tar_xz_data = Vec::new();
        {
            ...
            tar_builder.finish().map_err(LuppoError::IoError)?;
        }
        ...
        // Luppo standardı gereği SHA1 ana hash olarak atanır
        ...
        let luppo_root = LuppoRoot { source: None, package: metadata };
        let xml_content = quick_xml::se::to_string(&luppo_root).map_err(|e| {
            LuppoError::RuntimeError(t!("packager_error_xml_gen", error = e).to_string())
        })?;
        ...
        zip.start_file("install.tar.xz", zip::write::FileOptions::default())
            .map_err(|e| LuppoError::RuntimeError(e.to_string()))?;
        zip.write_all(&tar_xz_data).map_err(LuppoError::IoError)?;

        zip.finish()
            .map_err(|e| LuppoError::RuntimeError(e.to_string()))?;

        // 5. Oluşturulan paketin (.luppo) genel özetlerini hesapla
    ...
    pub fn parse_ar(data: &[u8]) -> LuppoResult<HashMap<String, Vec<u8>>> {
        ...
        while offset < data.len() {
            ...
            let name_str = std::str::from_utf8(&header[0..16])
                .map_err(|e| {
                    LuppoError::RuntimeError(t!("packager_err_ar_invalid_name", error = e).to_string())
                })?
                .trim();
            ...
            let size_str = std::str::from_utf8(&header[48..58])
                .map_err(|e| {
                    LuppoError::RuntimeError(t!("packager_err_ar_invalid_size", error = e).to_string())
                })?
                .trim();
            let size: usize = size_str.parse().map_err(|e| {
                LuppoError::RuntimeError(t!(
            ...
            if offset + size > data.len() {
                return Err(LuppoError::RuntimeError(t!("packager_err_ar_truncated").to_string()));
            }
    ...
    pub fn decompress_and_unpack_tar(
        data: &[u8],
    ) -> LuppoResult<Vec<(String, Vec<u8>, u32, u64, u64, Option<String>)>> {
        use std::io::Read;
        ...
        let decoded: Box<dyn Read> =
            if data.starts_with(&[0x1f, 0x8b]) {
                ...
                Box::new(zstd::stream::Decoder::new(data).map_err(|e| {
                    LuppoError::RuntimeError(t!("packager_err_zstd_decode", error = e).to_string())
                })?)
            } else {
                Box::new(data)
            };
        ...
        for entry_result in archive
            .entries()
            .map_err(|e| LuppoError::RuntimeError(t!("packager_err_tar_entries", error = e).to_string()))?
        {
            let mut entry = entry_result
                .map_err(|e| LuppoError::RuntimeError(t!("packager_err_tar_entry", error = e).to_string()))?;
            let path = entry
                .path()
                .map_err(|e| LuppoError::RuntimeError(t!("packager_err_tar_path", error = e).to_string()))?
                .to_string_lossy()
                .into_owned();
            ...
            let symlink_target = if entry.header().entry_type().is_symlink() {
                entry
                    .link_name()
                    .map_err(|e| {
                        LuppoError::RuntimeError(t!("packager_err_symlink_target", error = e).to_string())
                    })?
            ...
            if symlink_target.is_none() && entry.header().entry_type().is_file() {
                entry.read_to_end(&mut content).map_err(|e| {
                    LuppoError::RuntimeError(t!("packager_err_tar_content", error = e).to_string())
                })?;
            }
    ...
    /// Reads a .deb package and constructs a unified LuppoPackageData structure dynamically
    pub fn read_deb_package(path: &str) -> LuppoResult<LuppoPackageData> {
        let file_data = std::fs::read(path).map_err(LuppoError::IoError)?;
        ...
        let control_tar_key = ar_files
            ...
            .ok_or_else(|| {
                LuppoError::RuntimeError("deb paketi içinde control.tar bulunamadı".to_string())
            })?;
        ...
        let name = control_fields.get("Package").cloned().ok_or_else(|| {
            LuppoError::RuntimeError("control dosyasında Package adı bulunamadı".to_string())
        })?;
        ...
        let data_tar_key = ar_files
            ...
            .ok_or_else(|| {
                LuppoError::RuntimeError("deb paketi içinde data.tar bulunamadı".to_string())
            })?;
        ...
        let update = crate::package::Update {
            ...
            comment: "Debian package converted to Luppo".to_string(),
            name: m_name.clone(),
            email: m_email.clone(),
            type_: None,
            requires: None,
        ...
        let metadata = Package {
            ...
            partof: section,
            package_uri: format!("{}_deb.luppo", name),
            repo_url: String::new(),
            ...
            packager: Some(crate::package::Packager {
                name: m_name,
                email: m_email.unwrap_or_else(|| "info@antolun.com".to_string()),
            }),
            source: None,
            install_tar_hash: None,
            package_format: Some("1.2".to_string()),
            build_host: Some("local".to_string()),
            distribution: Some("LupuS".to_string()),
            version,
        };
...
mod tests {
    ...
    fn test_read_deb_package() {
        let deb_path = "/home/luppocik/uhapsigner_2.0.5_amd64.deb";
