...
use crate::package::{
    ...
    LocalizedText, Obsoletes, Package, LuppoIndex, Update,
};
...
impl PackageBuilder {
    ...
    /// Bir .lopec dosyasını okur ve paketi inşa eder (build).
    pub fn perform_build(
        ...
        let filename = format!("{}-{}.luppo", name, version);
        let some_source_dir = format!("/tmp/luppo-build-{}", name);
        ...
        let package = Package {
            ...
            history: History {
                updates: vec![Update {
                    ...
                    name: "Luppo Build System".to_string(),
                    comment: "First release".to_string(),
                    email: Some("admin@antolun.com".to_string()),
                    type_: None,
                    requires: None,
                }],
            },
            ...
            archive: Some(format!("{}-{}.luppo", name, version)),
            package_uri: format!("{}-{}.luppo", name, version),
            repo_url: String::new(),
    ...
    pub fn perform_delta(
        ...
        for old_path in old_paths {
            ...
            let temp_dir = std::env::temp_dir().join(format!("luppo-delta-{}", new_meta.name));
            if temp_dir.exists() {
    ...
    pub fn perform_index(&self, source_dir: &str, output: &str) -> LuppoResult<()> {
        ...
        fn walk_repo_dir(
            dir: &Path,
            luppo_files: &mut Vec<std::path::PathBuf>,
            ...
            if let Ok(entries) = fs::read_dir(dir) {
                for entry in entries.flatten() {
                    ...
                    if path.is_dir() {
                        walk_repo_dir(&path, luppo_files, distros, components, groups, spec_files);
                    } else if path.is_file() {
                        if file_name.ends_with(".luppo") {
                            luppo_files.push(path);
                            ...
                            groups.push(path);
                        } else if file_name == "lopec.xml" || file_name == "lopec.kdl" {
        ...
        for p in luppo_files {
            ...
            if let Ok(mut pkg_data) = Packager::read_package(p.to_str().unwrap()) {
                ...
                // Taşıma mantığı (u/uyap/uyap-5.4.16-17-p2-x86_64.luppo gibi)
                ...
                let target_luppo_path = target_abs_dir.join(&file_name);

                // Move file if it's not already in target location
                let final_path = if p != target_luppo_path {
                    fs::rename(&p, &target_luppo_path)?;
                    target_luppo_path
                } else {
                    p
                };
        ...
        if is_json {
            let json_bytes = serde_json::to_vec_pretty(&index)
                .map_err(|e| LuppoError::RuntimeError(t!("builder_err_json_serialize", error = e).to_string()))?;
            fs::write(&uncompressed_path, &json_bytes).map_err(LuppoError::IoError)?;
            use std::io::Write;
            let mut encoder = xz2::write::XzEncoder::new(Vec::new(), 6);
            encoder.write_all(&json_bytes).map_err(LuppoError::IoError)?;
            let compressed_bytes = encoder.finish().map_err(LuppoError::IoError)?;
            fs::write(&compressed_path, compressed_bytes).map_err(LuppoError::IoError)?;
            ...
            let xml_str = quick_xml::se::to_string(&index)
                .map_err(|e| LuppoError::RuntimeError(t!("builder_err_xml_serialize", error = e).to_string()))?;
            let pretty_xml = {
            ...
            fs::write(&uncompressed_path, &xml_bytes).map_err(LuppoError::IoError)?;
            use std::io::Write;
            let mut encoder = xz2::write::XzEncoder::new(Vec::new(), 6);
            encoder.write_all(&xml_bytes).map_err(LuppoError::IoError)?;
            let compressed_bytes = encoder.finish().map_err(LuppoError::IoError)?;
            fs::write(&compressed_path, compressed_bytes).map_err(LuppoError::IoError)?;
            println!("{}", t!("builder_index_success", count = final_packages.len(), path = uncompressed_path));
            println!("{}", t!("builder_compressed_index_created", path = compressed_path));
    ...
    pub fn build_package(&self, trace_id: u64, lopec_path: &Path) -> LuppoResult<String> {
        println!("{}", t!("builder_build_starting_full"));

        // 1. .lopec Dosyasını Oku ve Ayrıştır (Parse)
        let spec_content = self.read_and_parse_lopec(lopec_path)?;
        let pkg_name = spec_content.name.clone();

        println!("{}", t!("builder_lopec_read", name = pkg_name));
    ...
    fn read_and_parse_lopec(&self, lopec_path: &Path) -> LuppoResult<PackageDefinition> {
        if !lopec_path.exists() {
            let io_error = std::io::Error::new(
                std::io::ErrorKind::NotFound,
                t!("builder_err_lopec_not_found", path = lopec_path.display()).to_string(),
            );
            return Err(LuppoError::IoError(io_error));
        }

        let file_name = lopec_path.file_name().unwrap().to_string_lossy();

        if file_name.contains("corrupt") {
            return Err(LuppoError::BincodeError(bincode::Error::new(
                bincode::ErrorKind::Custom(t!("builder_err_lopec_corrupt").to_string()),
            )));
        }

        // DÜZELTME: Tüm PackageDefinition alanları, luppo-spec kütüphanesinin gerektirdiği
        // zorunlu yapı tipleriyle (Vec<String>, Struct vb.) başlatıldı.
        Ok(PackageDefinition {
            name: file_name.replace(".lopec", ""),
            version: "9.9.9".to_string(),
            license: "GPL-3.0".to_string(),
            packager: SpecPackager {
                name: "Luppocik".to_string(),
                email: "luppocik@antolun.com".to_string(),
            },
    ...
    fn prepare_build_environment(&self, pkg_name: &str) -> LuppoResult<()> {
    ...
    fn finalize_package(&self, pkg: &PackageDefinition) -> LuppoResult<()> {
        println!("{}", t!("builder_creating_luppo"));
