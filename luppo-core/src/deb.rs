...
impl DebManager {
    ...
    pub fn install_deb<P: AsRef<Path>>(&self, path: P, dest_root: &Path) -> Result<(), LuppoError> {
        ...
        let temp_dir = tempfile::tempdir().map_err(crate::LuppoError::IoError)?;
        ...
        let status = Command::new("ar")
            .arg("x")
            .arg(path.canonicalize().map_err(crate::LuppoError::IoError)?)
            .current_dir(temp_path)
            .status()
            .map_err(|e| crate::LuppoError::RuntimeError(t!("deb_err_ar", error = e).to_string()))?;

        if !status.success() {
            return Err(crate::LuppoError::RuntimeError(
        ...
        let data_archive = ["data.tar.xz", "data.tar.gz", "data.tar.zst", "data.tar.bz2"]
            ...
            .ok_or_else(|| crate::LuppoError::RuntimeError(t!("deb_err_no_data").to_string()))?;
        ...
        let status = Command::new("tar")
            ...
            .status()
            .map_err(|e| {
                crate::LuppoError::RuntimeError(t!("deb_err_tar", error = e).to_string())
            })?;
        ...
        let output = Command::new("tar")
            ...
            .arg(temp_path.join(data_archive))
            .output()
            .map_err(crate::LuppoError::IoError)?;
