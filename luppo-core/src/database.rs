...
impl Default for LuppoDatabase {
    fn default() -> Self {
        let config = Config::load(None);
        LuppoDatabase::open(config.directories.lib_dir.join("db"))
...
impl LuppoDatabase {
    ...
    pub fn open(path: PathBuf) -> LuppoResult<Self> {
    ...
    pub fn insert_source(&self, package_name: &str, lopec_url: &str) -> LuppoResult<()> {
        self.sources
            .insert(package_name.trim().as_bytes(), lopec_url.trim().as_bytes())?;
        Ok(())
    }

    pub fn get_source(&self, package_name: &str) -> LuppoResult<Option<String>> {
    ...
    pub fn insert_repo(&self, name: &str, entry: RepositoryEntry) -> LuppoResult<()> {
        let value = bincode::serialize(&entry).map_err(LuppoError::BincodeError)?;
        self.repositories.insert(name.as_bytes(), value)?;
        self.repositories.flush()?;
        Ok(())
    }

    pub fn remove_repo(&self, name: &str) -> LuppoResult<bool> {
    ...
    pub fn list_repos(&self) -> LuppoResult<Vec<RepositoryEntry>> {
        ...
        for item in self.repositories.iter() {
            let (_, value) = item?;
            repos.push(bincode::deserialize(&value).map_err(LuppoError::BincodeError)?);
        }
        Ok(repos)
    }

    pub fn get_repo_name_by_package_name(&self, package_name: &str) -> LuppoResult<String> {
        if let Some(ivec) = self.packages.get(package_name)? {
            let pkg: Package = bincode::deserialize(&ivec).map_err(LuppoError::BincodeError)?;
    ...
    pub fn install_package(&self, pkg: &InstalledPackage) -> LuppoResult<()> {
    ...
    pub fn remove_package(&self, name: &str) -> LuppoResult<()> {
    ...
    pub fn get_installed_package(&self, name: &str) -> LuppoResult<Option<InstalledPackage>> {
    ...
    pub fn list_installed_packages(&self) -> LuppoResult<Vec<InstalledPackage>> {
    ...
    pub fn list_installed_package_names(&self) -> LuppoResult<HashSet<String>> {
    ...
    pub fn list_available_package_names(&self) -> LuppoResult<HashSet<String>> {
        let mut names = HashSet::new();
    ...
    pub fn get_packages_by_names(&self, names: &HashSet<String>) -> LuppoResult<Vec<crate::package::Package>> {
    ...
    pub fn load_all_runtime_deps(&self) -> LuppoResult<HashMap<String, Vec<String>>> {
    ...
    pub fn get_or_build_runtime_deps_cache(&self) -> LuppoResult<HashMap<String, Vec<String>>> {
    ...
    pub fn invalidate_runtime_deps_cache(&self) -> LuppoResult<()> {
    ...
    pub fn is_package_installed(&self, name: &str) -> LuppoResult<bool> {
        Ok(self.installed.get(name.as_bytes())?.is_some())
    }

    pub fn clear_installed_packages(&self) -> LuppoResult<()> {
        self.installed.clear().map_err(LuppoError::DatabaseError)?;
        self.files.clear().map_err(LuppoError::DatabaseError)?;
        self.installed.flush()?;
        self.files.flush()?;
        Ok(())
    }

    pub fn clear_repo_packages(&self) -> LuppoResult<()> {
        self.packages.clear().map_err(LuppoError::DatabaseError)?;
        self.packages.flush().map_err(LuppoError::DatabaseError)?;
        Ok(())
    }

    // --- MEVCUT (DEPO) PAKETLERİ ---

    pub fn save_package(&self, pkg: &Package) -> LuppoResult<()> {
        let mut pkg_to_save = pkg.clone();
        pkg_to_save.prune();
        let compressed = Self::compress(&pkg_to_save)?;
        self.packages
    ...
    pub fn get_available_package(&self, name: &str) -> LuppoResult<Option<Package>> {
    ...
    pub fn get_package(&self, name: &str) -> LuppoResult<Option<Package>> {
        self.get_available_package(name)
    }

    pub fn list_available_packages(&self) -> LuppoResult<Vec<Package>> {
        let mut results = Vec::new();
        for item in self.packages.iter() {
            let (_, value) = item?;
    ...
    pub fn insert_package(&self, pkg: &PackageDefinition) -> LuppoResult<()> {
        let compressed = Self::compress(pkg)?;
        self.local_packages.insert(pkg.name.as_bytes(), compressed)?;
        Ok(())
    }

    pub fn search_package(&self, query: &str) -> LuppoResult<Vec<Package>> {
        let mut results = Vec::new();
    ...
    pub fn get_all_available_packages(&self) -> LuppoResult<Vec<Package>> {
        let mut packages = Vec::new();
        for item in self.packages.iter() {
            let (_key, value) = item.map_err(LuppoError::DatabaseError)?;
            if let Ok(pkg) = Self::decompress::<Package>(&value) {
                packages.push(pkg);
            }
        }
    ...
    pub fn save_component(&self, comp: &Component) -> LuppoResult<()> {
    ...
    pub fn get_component(&self, name: &str) -> LuppoResult<Option<Component>> {
    ...
    pub fn list_components(&self) -> LuppoResult<Vec<Component>> {
        let mut results = Vec::new();
    ...
    pub fn register_file(&self, file_path: &str, pkg_name: &str) -> LuppoResult<()> {
    ...
    pub fn find_package_by_file(&self, file_path: &str) -> LuppoResult<Option<String>> {
    ...
    pub fn remove_file_entry(&self, file_path: &str) -> LuppoResult<()> {
    ...
    pub fn get_files_for_package(&self, name: &str) -> LuppoResult<Option<Vec<FileData>>> {
        if let Some(data) = self.files.get(name).map_err(LuppoError::DatabaseError)? {
            let files: Vec<FileData> = bincode::deserialize(&data)
                .map_err(|e| LuppoError::DatabaseError(sled::Error::Unsupported(e.to_string())))?;
            Ok(Some(files))
        } else {
            Ok(None)
        }
    ...
    pub fn record_action(&self, action: HistoryAction) -> LuppoResult<()> {
    ...
    pub fn list_history(&self, filter_trace_id: Option<u64>) -> LuppoResult<Vec<HistoryAction>> {
    ...
    pub fn get_history(&self) -> LuppoResult<Vec<HistoryAction>> {
    ...
    pub fn delete_history_entry(&self, trace_id: u64) -> LuppoResult<()> {
    ...
    pub fn get_actions_for_rollback(&self, start_trace_id: u64) -> LuppoResult<Vec<HistoryAction>> {
    ...
    pub fn delete_package_only(&self, name: &str) -> LuppoResult<()> {
    ...
    pub fn get_next_trace_id(&self) -> LuppoResult<u64> {
    ...
    pub fn increment_trace_id(&self, current_id: u64) -> LuppoResult<()> {
    ...
    pub fn flush(&self) -> LuppoResult<usize> {
        self.db.flush().map_err(LuppoError::DatabaseError)
    }

    fn compress<T: Serialize>(data: &T) -> LuppoResult<Vec<u8>> {
        let encoded = bincode::serialize(data).map_err(LuppoError::BincodeError)?;
        zstd::encode_all(&encoded[..], 3).map_err(|e| LuppoError::RuntimeError(e.to_string()))
    }

    fn decompress<T: for<'a> Deserialize<'a>>(data: &[u8]) -> LuppoResult<T> {
        let decompressed =
            zstd::decode_all(data).map_err(|e| LuppoError::RuntimeError(e.to_string()))?;
        bincode::deserialize(&decompressed).map_err(LuppoError::BincodeError)
    }

    // --- BACKUP / RESTORE ---

    /// Veritabanını belirtilen dizine yedekler (dosya sistemi seviyesinde kopyalama)
    pub fn backup_to_dir(&self, backup_dir: &PathBuf) -> LuppoResult<()> {
    ...
    pub fn restore_from_dir(&self, backup_dir: &PathBuf) -> LuppoResult<()> {
        if !backup_dir.exists() {
        ...
        if trace_path.exists() {
            ...
            let trace_id: u64 = serde_json::from_str(&content)
                .map_err(|e| LuppoError::RuntimeError(e.to_string()))?;
            self.db.insert(LuppoDatabase::TRACE_ID_KEY, &trace_id.to_be_bytes())?;
        }
    ...
    fn copy_dir_all(src: &Path, dst: &Path) -> LuppoResult<()> {
        if !dst.exists() {
    ...
    pub fn verify_integrity(&self) -> LuppoResult<()> {
...
mod tests {
    ...
    fn setup_test_db() -> (LuppoDatabase, tempfile::TempDir) {
        let dir = tempdir().expect("Geçici dizin oluşturulamadı");
        let db = LuppoDatabase::open(dir.path().to_path_buf()).expect("Test veritabanı açılamadı");
        (db, dir)
    }
    ...
    fn test_repo_management() {
        ...
        let repo = RepositoryEntry {
            ...
            url: "https://repo.antolun.com".to_string(),
            mirrors: Vec::new(),
            enabled: true,
        };
    ...
    fn test_file_ownership_registry() {
        let (db, _dir) = setup_test_db();
        db.register_file("/etc/luppo.conf", "luppo").unwrap();

        let owner = db.find_package_by_file("/etc/luppo.conf").unwrap();
        assert_eq!(owner, Some("luppo".to_string()));

        db.remove_file_entry("/etc/luppo.conf").unwrap();
        assert!(db.find_package_by_file("/etc/luppo.conf").unwrap().is_none());
    }
