# luppo: Luppo Paket Yönetim Sistemi (Rust Sürümü)

Luppo (Packages Installed Successfully as Intended), LupuS projeleri tarafından geliştirilen modern bir paket yönetim sistemidir. `luppo`, bu sistemi Rust programlama dili ile yeniden hayata geçirmeyi hedefleyen, yüksek performanslı ve güvenli bir sürümüdür.

## Özellikler ve Karşılaştırma

| Özellik           | Python Luppo (Eski)        | luppo (Yeni)                          |
| :---------------- | :------------------------- | :------------------------------------ |
...
| **Paket Formatı** | .luppo (ZIP + XML)         | .luppo (Geriye dönük tam uyumlu)      |
...
| python3          | system/base/python3          | Python 3 helper modülleri                  |
| catbox           | system/devel/catbox          | luppo sandbox'ı Rust'ta                    |
| python-six       | system/base/python-six       | Python 3 uyumluluk                         |
...
| Paket              | Dizin                          | Açıklama                                        |
| ------------------ | ------------------------------ | ----------------------------------------------- |
| python3            | system/base/python3            | luppo pyo3 ile Python 3 actions.py çalıştırır   |
...
<komut> aşağıdakilerden birisi olabilir:
    ...
    [+]            history (hs) - Luppo işlemlerinin günlüğü (Renkli ve korumalı rollback desteğiyle)
    [+]              index (ix) - Verilen dizindeki Luppo dosyalarının kataloğunu çıkar
    ...
    [+]             remove (rm) - Luppo paketlerini kaldır
...
Belirli bir komut hakkında yardım almak için "luppo help <komut>" kullanınız.
...
[+] -h [--help] : bu yardım iletisini göster ve çık
    ...
    genel seçenekler:
    [+]	-D [--destdir] arg          : Luppo komutları için sistem kökünü değiştir.
    ...
    [+]	-N [--no-color]             : Luppo çıktılarında renk kullanılmasını engeller.
...
`luppo` Rust workspace yapısında 4 ana crate'den oluşur:

| Crate              | Yol              | Açıklama                                                                                                                            |
| ------------------ | ---------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **luppo-spec**     | `luppo-spec/`    | Paket şemaları ve model tipleri (XML/KDL parsing, `Package`, `PackageActions`, `BuildDependencies`, `BuildDepsVisitor`)             |
| **luppo-core**     | `luppo-core/`    | Çekirdek mantık: `Resolver`, `Installer`, `Package`, `InstalledPackage`, `Repo`, `Query`, `History`, `Lock`, `Delta`, GPG doğrulama |
| **luppo-builder**  | `luppo-builder/` | Paket inşa motoru: `BuildOptions`, `internal_build`, sandbox/host kurulum, actionsapi                                               |
| **luppo** (binary) | `luppo/`         | CLI entry point: `Commands`, arg parsing (Clap), Python API bridge, komut dispatch                                                  |

### luppo-builder / actionsapi Modül Yapısı

`luppo-builder/src/actionsapi/` altındaki modüler yapı (eski 1732 satırlık `actionsapi.rs` yerine):
...
- **Build Dep Yönetimi**: Python `luppo bi` davranışı birebir taklit edildi — unsatisfied build deps listesi → onay → transitif çözüm → tam plan → onay → kurulum (sandbox/host)
...
- **Delta Paket**: Hesaplama mantığı var, `.delta.luppo` dosya yazma ve installer uygulaması eksik (ileride tamamlanacak)

### Derleme & Test

```bash
cargo build          # Tüm workspace
cargo test           # 11 test (luppo-spec 4, luppo-builder 7)
cargo build --release
```
