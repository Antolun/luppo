...
// ROOT MODÜL — pisi.actionsapi olarak Python'a açılır
#[pymodule]
...
pub fn init_actionsapi_module(py: Python, m: &PyModule) -> PyResult<()> {
    ...
    let pisitools_mod = PyModule::new(py, "pisitools")?;
    actionsapi::pisitools::init_module(py, pisitools_mod)?;
    m.add_submodule(pisitools_mod)?;
