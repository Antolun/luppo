...
pub fn reset_history(path: &Path) -> Result<(), String> {
    ...
    // ── XML pspec dosyalarını bul ve işle (varsayılan) ──
    let mut pspec_files = Vec::new();
    find_pspec_files(&canonical_path, "pspec.xml", &mut pspec_files).map_err(|e| e.to_string())?;
    ...
    // KDL pspec dosyalarını bul ve işle
    let mut kdl_files = Vec::new();
    find_pspec_files(&canonical_path, "pspec.kdl", &mut kdl_files).map_err(|e| e.to_string())?;
    for f in kdl_files {
        let relative = f.strip_prefix(&canonical_path).unwrap();
        let backup_file_path = backup_dir.join(relative);
        if let Some(parent) = backup_file_path.parent() {
...
fn find_pspec_files(dir: &Path, filename: &str, files: &mut Vec<PathBuf>) -> std::io::Result<()> {
    for entry in fs::read_dir(dir)? {
        let entry = entry?;
        let path = entry.path();
        if path.is_dir() {
            find_pspec_files(&path, filename, files)?;
...
fn do_reset_history_kdl(content: &str) -> Result<String, String> {
    let spec: PisiSpec = pisi_spec::kdl::parse_kdl_spec_from_str(content)?;
...
fn make_history_block(first: &pisi_spec::models::Update, indent: &str) -> String {
...
"#;
        ...
        let spec = pisi_spec::kdl::parse_kdl_spec_from_str(&result).unwrap();
