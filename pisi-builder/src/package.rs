use pisi_spec::models::{PathDef, PisiSpec};
use rust_i18n::t;
...
/// Python PiSi'nin `package_filename()` fonksiyonuyla uyumlu paket adı üretir.
/// Format: <name>-<version>-<release>-<distro_id>-<arch>.pisi
...
pub fn check_path_collision(
    pkg_idx: usize,
    packages: &[pisi_spec::models::PackageDefinition],
...
/// Belirtilen paket için .pisi arşivi oluşturur.
///
/// - `install_root`: Bu paketin dosyalarının bulunduğu dizin
/// - `output_dir`: .pisi dosyasının yazılacağı dizin (pkg_dir)
...
pub fn create_pisi_package(
    ...
    let pisi_filename = make_package_filename(&pkg.name, version, release, distro_id, arch);
    let pisi_path = output_dir.join(&pisi_filename);
    ...
    // 2. InstalledSize: Sıkıştırılmamış dosya boyutları toplamı (Python PiSi uyumu)
    ...
    println!("📦 Paket oluşturuldu: {}", pisi_path.display());
    Ok(pisi_path)
}

/// Python PiSi'nin `gen_files_xml()` mantığıyla uyumlu dosya toplama.
...
fn collect_package_files(
    ...
    // Yola göre sırala (Python PiSi lzma sıkıştırmayı iyileştirmek için sıralıyor)
    files.sort_by(|a, b| a.path.cmp(&b.path));
    Ok(files)
}
...
fn create_zip_archive(
    ...
    metadata_xml: &str,
    specdir: &Path,
    _pkg: &pisi_spec::models::PackageDefinition,
) -> Result<(), String> {
    let file = File::create(zip_path).map_err(|e| format!("Failed to create .pisi file: {}", e))?;
...
fn generate_metadata_xml(
    ...
    // Summary ve Description: pakette yoksa source'tan al (Python PiSi uyumu)
    ...
    let distribution = "PisiLinux";
    let distribution_release = "2.0";
    ...
    // Paket kopyası olarak Source tekrarı (Python PiSi uyumu)
    xml.push_str("        <Source>\n");
    xml.push_str(&format!("            <Name>{}</Name>\n", spec.source.name));
    if let Some(ref p) = spec.source.packager {
        xml.push_str("            <Packager>\n");
