...
class Install(AtomicOperation):
    ...
    def extract_install(self):
        ...
        if self.reinstall():
            ...
            new = list(filter(lambda x: x.type == 'config', self.files.list))
            old = list(filter(lambda x: x.type == 'config', self.old_files.list))
            ...
            if config_overlaps:
                files = list(filter(lambda x: x.path in config_overlaps, old))
                for f in files:
