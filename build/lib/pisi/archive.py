...
class TarFile(tarfile.TarFile):
    ...
    def lzmaopen(cls,
        ...
        if fileobj is not None:
            ...
            #           "level":     compresslevel}
            # In Python 3 lzma.open is better
            fileobj = lzma.open(name, mode, preset=compresslevel)
...
class ArchiveBzip2(ArchiveBase):
    ...
    def unpack_dir(self, target_dir):
        ...
        import bz2
        with bz2.BZ2File(self.file_path, "r") as bz2_file:
...
class ArchiveGzip(ArchiveBase):
    ...
    def unpack_dir(self, target_dir):
        ...
        import gzip
        with gzip.GzipFile(self.file_path, "r") as gzip_file:
            with open(output_path, "wb") as output:
...
class ArchiveLzma(ArchiveBase):
    ...
    def unpack_dir(self, target_dir):
        ...
        import lzma
        with lzma.LZMAFile(self.file_path, "r") as lzma_file:
            with open(output_path, "wb") as output:
...
class ArchiveZip(ArchiveBase):
    ...
    symmagic = 2716663808  # int of hex val '0xA1ED0000'
    ...
    def add_to_archive(self, file_name, arc_name=None):
        ...
        if os.path.isdir(file_name) and not os.path.islink(file_name):
            ...
            attr_obj.external_attr = stat.S_IMODE(os.stat(file_name).st_mode) << 16
