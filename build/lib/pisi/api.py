...
def locked(func):
    ...
    def wrapper(*__args,**__kw):
        try:
            lock_path = pisi.util.join_path(pisi.context.config.lock_dir(), 'pisi')
...
def index(dirs=None, output='pisi-index.xml',
    ...
    index_obj = pisi.index.Index()
    index_obj.distribution = None
    if not dirs:
        dirs = ['.']
    for repo_dir in dirs:
        repo_dir = str(repo_dir)
        ctx.ui.info(_('Building index of PiSi files under %s') % repo_dir)
        index_obj.index(repo_dir, skip_sources)

    sign = None if skip_signing else pisi.file.File.detached
    index_obj.write(output, sha1sum=True, compress=compression, sign=sign)
...
def add_repo(name, indexuri, at = None):
    ...
    if repodb.has_repo(name):
        ...
        repo_name = repodb.get_repo_by_url(indexuri)
        raise pisi.Error(_('Repo already present with name %s.') % repo_name)
    else:
...
def __update_repo(repo, force=False):
    ...
    if repodb.has_repo(repo):
        ...
        try:
            index_obj.read_uri_of_repo(repouri, repo)
            ...
            if force:
                ...
                index_obj.read_uri_of_repo(repouri, repo, force = force)
            else:
                return False
        ...
        try:
            index_obj.check_signature(repouri, repo)
...
def rebuild_db(files=False):
    ...
    def rebuild_filesdb():
        for pkg in list_installed():
            ...
            files_obj = installdb.get_files(pkg)
            filesdb.add_files(pkg, files_obj)
            ctx.ui.info(_('OK.'))
...
def clearCache(all=False):
    ...
    def getPackageLists(pkgList):
        ...
        for f in pkgList:
            try:
                ...
                version, release, build_val = pisi.util.split_version(full_version)
    ...
    if not all:
        ...
        cacheLimit = int(config.get("general", "package_cache_limit") or 0) * 1024 * 1024
        if not cacheLimit:
            return
