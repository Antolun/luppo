...
class CLI(pisi.ui.UI):
    ...
    def confirm(self, msg):
        ...
        import re, tty, termios

        # Note: nl_langinfo might fail or return bytes depending on locale
        try:
            ...
            # Fallback for systems with poor locale support
            yes_expr = re.compile(r"^[yY]")
            no_expr = re.compile(r"^[nN]")
    ...
    def display_progress(self, **ka):
        ...
        if ka['operation'] in ["removing", "rebuilding-db"]:
            ...
            out = '\r%-30.50s (%s)%3d%% %9.2f %s [%s]' % \
                (ka['filename'], totalsize, int(ka['percent']),
                 ka['rate'], ka['symbol'], ka['eta'])
            self.output(out)
        else:
            self.output("\r%s (%d%%)" % (ka['info'], int(ka['percent'])))
