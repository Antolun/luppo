...
""")
    ...
    def options(self, group):
        group.title = _("upgrade options")

        super(Upgrade, self).options(group)
        group.add_argument("--security-only", action="store_true",
                     default=False, help=_("Security related package upgrades only"))
        group.add_argument("-b", "--bypass-update-repo", action="store_true",
        ...
        group.add_argument("-r", "--repository", action="store",
                               type=str, default=None, help=_('Name of the to be upgraded packages\' repository'))
