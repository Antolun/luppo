# Luppo Paketçi Kılavuzu

Bu kılavuz, LupuS için KDL formatında paket tarifleri hazırlamak, mevcut tarifleri güncellemek ve servis tanımları oluşturmak isteyen paketçiler için hazırlanmıştır.

---

## İçindekiler

1. [Luppo Paket Klasör Yapısı](#1-luppo-paket-klasör-yapısı)
2. [KDL Formatına Giriş](#2-kdl-formatına-giriş)
3. [LuppoPackage Şeması](#3-luppopackage-şeması)
...
Luppo, paket tanımları için **KDL** (KDL Document Language) formatını kullanır. KDL, XML'e göre daha yalın ve insan okunabilir bir yapı sunar.
...
`LuppoPackage` en üst düzey kapsayıcıdır. İçinde bir adet `Source`, bir veya daha fazla `Package` ve isteğe bağlı `History` bulunur.
...
Actions {
    ...
    steps "luppotools.dodoc('README', 'LICENSE')"
}
```
...
ActionsAPI, KDL içinde `autotools.configure()`, `luppotools.dobin()`, `cargotools.build()` gibi fonksiyonları kullanmanızı sağlar.
...
| Kullanım                                           | Açıklama                                                          |
| -------------------------------------------------- | ----------------------------------------------------------------- |
| `luppotools.dobin(dosya, /bin)`                    | Çalıştırılabiliri `/usr/bin` (veya belirtilen) hedefe kopyalar    |
| `luppotools.dodir(/usr/include/awk)`               | Install dizininde klasör oluşturur                                |
| `luppotools.dodoc(README, ChangeLog)`              | Dosyaları `/usr/share/doc/<paket>/` altına kopyalar               |
| `luppotools.doexe(dosya, /etc/scripts)`            | Dosyayı çalıştırılabilir olarak kopyalar                          |
| `luppotools.dohtml(index.html)`                    | HTML'leri `/usr/share/doc/<paket>/html/` altına kopyalar          |
| `luppotools.doinfo(*.info)`                        | Info dosyalarını `/usr/share/info/` altına kopyalar               |
| `luppotools.dolib(libz.a, /lib)`                   | Kütüphaneyi hedefe kopyalar                                       |
| `luppotools.dolib_a(libpci.a)`                     | Statik (`.a`) kütüphaneyi uygun izinle kopyalar                   |
| `luppotools.dolib_so(libdb.so)`                    | Paylaşımlı (`.so`) kütüphaneyi uygun izinle kopyalar              |
| `luppotools.doman(logrotate.8)`                    | Man sayfalarını `/usr/share/man/` altına kopyalar                 |
| `luppotools.domo(po/tr.po, tr, app.mo)`            | PO'yu MO'ya derleyip `/usr/share/locale/...` altına kopyalar      |
| `luppotools.domove(/usr/bin/passwd, /bin/)`        | Install dizini içinde dosya taşır/yeniden adlandırır              |
| `luppotools.dosed(Makefile, -O3, %cflags%)`        | Work dizinindeki dosyada sed ile değişiklik yapar                 |
| `luppotools.dosbin(traceroute6)`                   | Sistem yöneticisi çalıştırılabilirini `/usr/sbin` altına kopyalar |
| `luppotools.dosym(gzip, /bin/gunzip)`              | Install dizininde sembolik link oluşturur                         |
| `luppotools.insinto(/etc/, nanorc.sample, nanorc)` | Dosyayı korunmuş izinlerle hedefe kopyalar                        |
| `luppotools.newdoc(README, README.new)`            | Doc dizinine farklı isimle kopyalar                               |
| `luppotools.newman(less.nro, less.1)`              | Man dizinine farklı isimle kopyalar                               |
| `luppotools.remove(/usr/lib/bad.so)`               | Install dizininden dosya siler                                    |
| `luppotools.rename(/usr/bin/bash, bash.old)`       | Install dizininde dosya adını değiştirir                          |
| `luppotools.removeDir(/usr/libexec)`               | Install dizinindeki klasörü tüm içeriğiyle siler                  |
| `luppotools.installHeaders(include/*.h)`           | Header dosyalarını install dizinine kopyalar                      |
...
component "system" {
    ...
    group "system"
    maintainer-name "Antolun Community"
    maintainer-email "admins@antolun.com"
}
...
distribution {
    source-name "LupuS"
    version "all"
    description lang="tr" "LupuS Varsayılan Deposu"
    description lang="en" "LupuS Default Repository"
    type "Default"
    binary-name "LupuS"
    obsoletes {
        package "glibc-32bit"
...
LuppoPackage {
    ...
    Package {
        ...
        Actions {
            steps "make"
            steps "luppotools.dobin(disktype, /usr/bin)"
            steps "luppotools.doman(disktype.1)"
        }
    }
    History {
...
Bu kılavuzdaki standartlara uyarak LupuS için temiz, kararlı ve modern paketler hazırlayabilirsiniz.
