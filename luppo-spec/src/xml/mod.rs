...
/// Belirtilen XML dosyasını ayrıştırır ve bir LuppoSpec yapısı döndürür.
...
pub fn parse_xml_spec<P: AsRef<Path>>(path: P) -> Result<LuppoSpec, String> {
    let path_ref = path.as_ref();
    let content = fs::read_to_string(path_ref).map_err(|e| e.to_string())?;

    let content = sanitize_xml_for_serde(&content);

    validate_lopec(&content)?;

    let spec: LuppoSpec = quick_xml::de::from_str(&content)
        .map_err(|e| {
...
pub fn validate_lopec(content: &str) -> Result<(), String> {
...
mod tests {
    ...
    fn test_validate_valid_lopec() {
        ...
        "#;
        assert!(validate_lopec(xml).is_ok());
    }
    ...
    #[test]
    fn test_parse_libreoffice_lopec() {
        let path = "/media/luppocuk/REPO/LupuS/LupuS_docker/main/office/libreoffice/libreoffice/lopec.xml";
        ...
        match parse_xml_spec(path) {
            Ok(spec) => {
                ...
                println!("LibreOffice lopec.xml parsed successfully: {} packages, {} history entries",
                    spec.packages.len(),
    ...
    <Package>
        ...
        // raw fails
        assert!(quick_xml::de::from_str::<models::LuppoSpec>(xml).is_err());
        // sanitized works
        let sanitized = sanitize_xml_for_serde(xml);
        assert!(quick_xml::de::from_str::<models::LuppoSpec>(&sanitized).is_ok());
    }

    #[test]
    fn test_strip_doctype_and_decl() {
        let xml = r#"<?xml version="1.0" ?>
<!DOCTYPE LUPPO SYSTEM "https://example.dtd">
<LUPPO>
    <Source>
        <Name>test</Name>
        <Version>1.0</Version>
        ...
        assert!(cleaned.starts_with("<LUPPO>"), "Expected <LUPPO> but got: {:?}", &cleaned[..cleaned.len().min(20)]);

        // Verify it parses
        let spec: Result<LuppoSpec, _> = quick_xml::de::from_str(&cleaned);
        assert!(spec.is_ok(), "Parse failed: {:?}", spec.err());
    }

    #[test]
    fn test_doctype_on_same_line_as_decl() {
        let xml = r#"<?xml version="1.0"?> <!DOCTYPE LUPPO SYSTEM "luppo.dtd">
<LUPPO>
    <Source>
        <Name>test</Name>
        <Version>1.0</Version>
        ...
        assert!(cleaned.starts_with("<LUPPO>"), "Expected <LUPPO> but got: {:?}", &cleaned[..cleaned.len().min(60)]);
        let spec: Result<LuppoSpec, _> = quick_xml::de::from_str(&cleaned);
    ...
    fn test_doctype_only_on_same_line() {
        let xml = r#"<!DOCTYPE LUPPO SYSTEM "luppo.dtd">
<LUPPO>
    <Source>
        <Name>test</Name>
        <Version>1.0</Version>
...
<!DOCTYPE LUPPO SYSTEM "luppo.dtd">
<LUPPO>
    <Source>
        <Name>test</Name>
        <Version>1.0</Version>
    ...
    fn test_bom_and_doctype_same_line_with_comment() {
        let xml = format!("\u{FEFF}{}", r#"<?xml version="1.0"?> <!DOCTYPE LUPPO SYSTEM "luppo.dtd">
<!-- eh -->
<LUPPO>
    <Source>
        <Name>test</Name>
...
<LUPPO>
    <Source>
        ...
        assert!(cleaned.starts_with("<LUPPO>"), "Expected <LUPPO> but got: {:?}", &cleaned[..cleaned.len().min(80)]);
        let spec: Result<LuppoSpec, _> = quick_xml::de::from_str(&cleaned);
        assert!(spec.is_ok(), "Parse failed: {:?} [preview: {:?}]", spec.err(), &cleaned[..cleaned.len().min(200)]);
    }
    ...
    fn test_new_fields_parsing() {
        ...
        let spec: LuppoSpec = quick_xml::de::from_str(xml).unwrap();
