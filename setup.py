...
class Build(build):
    def run(self):
        ...
        for in_file in IN_FILES:
            ...
            out_file = os.path.join(self.build_base, name)
            if shutil.which("intltool-merge") is not None:
                self.spawn(["intltool-merge", "-x", "po", in_file, out_file])
            else:
                print("Warning: 'intltool-merge' not found, skipping %s generation." % out_file)
                print("  To fix: sudo pisi it intltool  (or equivalent for your distro)")
                # Copy the .in file as a fallback so build doesn't fail
...
class Install(install):
    ...
    def installi18n(self):
        for name in os.listdir('po'):
            ...
            subprocess.run(["msgfmt", "po/%s.po" % lang, "-o", "po/%s.mo" % lang], check=True)
            if not self.root:
                self.root = "/"
...
setup(name="pisi",
    ...
    scripts = ['scripts/lspisi', 'scripts/unpisi', 'scripts/check-newconfigs.py', 'scripts/revdep-rebuild'],
    entry_points = {
        'console_scripts': [
            'pisi = pisi.cli.pisicli:main',
        ],
    },
