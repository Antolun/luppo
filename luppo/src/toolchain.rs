...
use luppo_core::{
    config::Config, database::LuppoDatabase, installer::Installer, LuppoError, LuppoResult,
};
...
fn mount_fs(source: &str, target: &str, fstype: &str, flags: &str) -> Result<(), LuppoError> {
    ...
    let status = cmd.status().map_err(|e| {
        LuppoError::RuntimeError(format!("{}: {}", t!("toolchain_mount_cmd_fail"), e))
    })?;
    if !status.success() {
...
pub fn perform_toolchain_start(_trace_id: u64) -> LuppoResult<()> {
    ...
    for dir in &dirs {
        ...
        if !path.exists() {
            ...
            fs::create_dir_all(&path).map_err(LuppoError::IoError)?;
        }
    }
...
pub fn perform_toolchain_update(trace_id: u64) -> LuppoResult<()> {
    if !is_root() {
        return Err(LuppoError::RuntimeError(
            t!("error_root_required").to_string(),
        ));
    ...
    let db_path = "/var/lib/luppo/luppo.db";
    let db = LuppoDatabase::open(PathBuf::from(db_path))
        .map_err(|e| LuppoError::RuntimeError(t!("toolchain_err_db", error = e).to_string()))?;
    ...
    for (idx, pkg_name) in chroot_packages.iter().enumerate() {
        ...
        // Paket tarifi (lopec.kdl veya lopec.xml) dosyasını ara
        let recipe_paths = [
            format!("./recipes/{}/lopec.kdl", pkg_name),
            format!("./recipes/{}/lopec.xml", pkg_name),
            format!("/mnt/chroot/recipes/{}/lopec.kdl", pkg_name),
            format!("/mnt/chroot/recipes/{}/lopec.xml", pkg_name),
            format!("/var/lib/luppo/recipes/{}/lopec.kdl", pkg_name),
            format!("/var/lib/luppo/recipes/{}/lopec.xml", pkg_name),
        ];
        ...
        if let Some(spec_path) = spec_file {
            ...
            let spec = LuppoSpec::from_path(&spec_path)
                .map_err(|e| LuppoError::RuntimeError(t!("toolchain_err_spec_parse", error = e).to_string()))?;
    ...
    // tar -C /mnt/chroot -c . | docker import - luppo-linux-chroot:latest
