...
use luppo_builder::build::{BuildOptions, PackageBuilder};
use luppo_spec::models::LuppoSpec; // Added for Build command
use std::fs;
use std::fs::File;
...
use luppo_core::{
    ...
    database::LuppoDatabase, installer::Installer, query::QueryManager, repo::Repository, LuppoError,
};
...
#[derive(Parser, Debug)]
#[command(author, version, about = "Rust-based Luppo Package Manager", long_about = None, disable_help_subcommand = true)]
pub struct LuppoCli {
    #[command(subcommand)]
    command: Commands,

    /// Change system root for Luppo commands
    ...
    /// Disable color in Luppo outputs
...
enum Commands {
    ...
    EmergeUp,
    #[command(visible_alias = "rm", about = "Remove Luppo packages")]
    Remove {
        package_name: String,
        /// Remove a Debian package instead of a Luppo package
        #[arg(long)]
        deb: bool,
    },
    ...
    #[command(visible_alias = "bi", about = "Build a new Luppo package")]
    Build {
        lopec_path: Option<PathBuf>,
    ...
    #[command(about = "Reset <History> tag in lopec.xml files to first release")]
    ResetHistory {
        #[arg(default_value = ".")]
        path: PathBuf,
    },
    ...
    ListFiles {
        /// Paket adı veya .luppo dosyası yolu
        package_name: String,
    },
    #[command(visible_alias = "hs", about = "History of Luppo operations")]
    History {
        #[arg(short = 't', long)]
        trace_id: Option<u64>,
    ...
    #[command(
        visible_alias = "ix",
        about = "Create a catalog of Luppo files in the given directory"
    )]
    Index {
        /// Luppo dosyalarının bulunduğu dizin
        #[arg(default_value = ".")]
        source_dir: String,
        /// Oluşturulacak indeks dosyası adı
        #[arg(short, long, default_value = "luppo-index.xml")]
        output: String,
    },
    ...
    #[command(
        visible_alias = "bd",
        about = "Backup Luppo database to a directory"
    )]
    BackupDb {
        /// Target backup directory
        #[arg(default_value = "/var/backup/luppo")]
        directory: String,
    },

    #[command(
        visible_alias = "rd",
        about = "Restore Luppo database from a backup directory"
    )]
    RestoreDb {
...
fn main() -> LuppoResult<()> {
    ...
    let cli = <LuppoCli as clap::FromArgMatches>::from_arg_matches(&matches)
        .map_err(|e| LuppoError::RuntimeError(e.to_string()))?;

    if cli.no_color {
        luppo_core::safe_env::set_var("NO_COLOR", "1");
    }
    ...
    if let Some(j) = &cli.jobs {
        ...
        if !j_val.is_empty() && j_val.chars().all(|c| c.is_ascii_digit()) {
            ...
            luppo_core::safe_env::set_var("MAKEOPTS", format!("-j{}", j_val));
        }
    }
    ...
    match &cli.command {
        ...
        Commands::CheckComponents { path, fix, edit } => {
            if *edit {
                luppo_builder::components::edit_components(path)?;
            } else {
                luppo_builder::components::check_components(path, *fix)?;
            }
            return Ok(());
        }
        Commands::ResetHistory { path } => {
            luppo_builder::reset_history::reset_history(path)?;
            return Ok(());
        }
        _ => {}
    }
    ...
    if !requires_lock && nix::unistd::geteuid().as_raw() != 0 {
        let temp_db_dir =
            std::env::temp_dir().join(format!("luppo-db-readonly-{}", nix::unistd::getuid().as_raw()));
    ...
    if let Some(parent) = db_path.parent() {
        if !parent.exists() {
            std::fs::create_dir_all(parent).map_err(LuppoError::IoError)?;
        }
    }

    let db = LuppoDatabase::open(db_path)?;
    ...
    match cli.command {
        ...
        Commands::Remove { mut package_name, deb } => {
            if package_name.ends_with(".deb") {
                if let Ok(pkg_data) = luppo_core::packager::Packager::read_package(&package_name) {
        ...
        Commands::Build { ref lopec_path, .. } => {
            // Otomatik algılama: lopec_path verilmemişse cwd'de lopec.xml/kdl ara
            let lopec_path = match lopec_path {
                Some(p) => p.clone(),
                None => {
                    let cwd = std::env::current_dir().map_err(LuppoError::IoError)?;
                    let candidates = ["lopec.kdl", "lopec.xml"];
                    let found = candidates.iter().find(|f| cwd.join(f).exists());
                    match found {
                        Some(f) => cwd.join(f),
                        None => {
                            return Err(LuppoError::SpecError(
                                "lopec.kdl veya lopec.xml bulunamadı.".to_string()
                            ));
                        }
                    }
                }
            ...
            let lopec_str = lopec_path.to_string_lossy();
            let mut target_url = lopec_str.to_string();

            // Eğer yerel bir dosya değilse ve URL de değilse veritabanından uzak kaynak URL'sini sorgula
            if !lopec_path.exists()
                && !lopec_str.starts_with("http://")
                && !lopec_str.starts_with("https://")
            {
                if let Ok(Some(url)) = db.get_source(&lopec_str) {
                    println!(
                        "{}",
                        t!("build_remote_found", package = lopec_str, url = url)
                    );
                    target_url = url;
                }
            }

            let local_lopec_path =
            ...
            println!(
                "{}",
                t!("build_starting", package = local_lopec_path.display())
            );

            let spec = LuppoSpec::from_path(&local_lopec_path).map_err(|e| {
                LuppoError::SpecError(t!("main_error_spec_parse", error = e).to_string())
            })?;

            // Validate date format in history
            if let Some(ref history) = spec.history {
                let raw_content =
                    fs::read_to_string(&local_lopec_path).map_err(LuppoError::IoError)?;
                ...
                for update in &history.updates {
                    if NaiveDate::parse_from_str(&update.date, "%Y-%m-%d").is_err() {
                        let line_num = lines
                            ...
                            .map(|i| i + 1)
                            .unwrap_or(0);
                        return Err(LuppoError::SpecError(t!(
            ...
            let original_dir = std::env::current_dir().map_err(LuppoError::IoError)?;

            // Python Luppo'deki pkg_dir() mantığıyla uyumlu dizin yap:
            ...
            if has_phase_flags {
                ...
                fs::create_dir_all(&work_dir).map_err(LuppoError::IoError)?;
            } else {
                // Temiz bir build: dizini silip yeniden oluştur
                if work_dir.exists() {
                    fs::remove_dir_all(&work_dir).map_err(LuppoError::IoError)?;
                }
                fs::create_dir_all(&work_dir).map_err(LuppoError::IoError)?;
            }

            // specdir: lopec/kdl dosyasının bulunduğu dizin
            let specdir = local_lopec_path
                .parent()
            ...
            if actions_py.is_file() {
                fs::copy(&actions_py, work_dir.join("actions.py")).map_err(LuppoError::IoError)?;
            ...
            for patch in patches {
                ...
                if src_patch.is_file() {
                    ...
                    if let Some(p) = dst_patch.parent() {
                        fs::create_dir_all(p).map_err(LuppoError::IoError)?;
                    }
                    fs::copy(&src_patch, &dst_patch).map_err(LuppoError::IoError)?;
                } else {
                    ...
                    if alt_src.is_file() {
                        ...
                        fs::copy(&alt_src, &dst_patch).map_err(LuppoError::IoError)?;
                    }
                }
            }
            ...
            if let Err(_e) = &res {
                let log_path = work_dir.join("luppo-build-error.log");
                if log_path.exists() {
            ...
            for built_package_path in built_package_paths {
                ...
                let final_package_path =
                    original_dir.join(built_package_path.file_name().ok_or_else(|| {
                        LuppoError::RuntimeError(t!("main_error_pkg_name").to_string())
                    })?);
...
fn acquire_lock(config: &Config) -> LuppoResult<File> {
    let lock_path = config.directories.lock_dir.join("luppo.lock");
    ...
    let file = File::create(&lock_path).map_err(LuppoError::IoError)?;
    ...
    if let Err(e) = flock(fd, FlockArg::LockExclusiveNonblock) {
        ...
        return Err(LuppoError::IoError(err));
    }
    Ok(file)
}

fn handle_version() -> LuppoResult<()> {
...
fn handle_help(command: Option<String>) -> LuppoResult<()> {
    let mut cmd = LuppoCli::command();
    cmd = localize_command(cmd);
...
fn handle_emerge(
    ...
    if !to_install_binary.is_empty() {
        ...
        luppo_core::print_in_columns(&to_install_binary);
    }

    if !to_build_source.is_empty() {
        println!("\n{}", t!("emerge_plan_build_header"));
        luppo_core::print_in_columns(&to_build_source);
    }
    ...
    for pkg_name in build_order {
        ...
        if let Some(source_url) = source_url_opt {
            ...
            let local_lopec_path = repo_manager.download_remote_spec(&source_url)?;
            let spec = LuppoSpec::from_path(&local_lopec_path).map_err(|e| {
                LuppoError::SpecError(t!("main_error_spec_parse", error = e).to_string())
            })?;
            ...
            let build_options = BuildOptions {
                ...
                yes_all,
                sbindir: std::env::var("LUPPO_SBINDIR").unwrap_or_else(|_| "usr/bin".to_string()),
                run_build: false,
                run_install: false,
                run_package: false,
            };

            let _original_dir = std::env::current_dir().map_err(LuppoError::IoError)?;
            ...
            let res = builder.build(trace_id);
            if let Err(_e) = &res {
                let log_path_err = work_dir.join("luppo-build-error.log");
...
fn handle_temp() -> LuppoResult<()> {
    ...
    fs::create_dir_all(dir_path.join("files")).map_err(LuppoError::IoError)?;
    fs::create_dir_all(dir_path.join("comar")).map_err(LuppoError::IoError)?;

    let template_content = include_str!("../luppo-template/template.kdl");
    let kdl_path = dir_path.join(format!("{}.kdl", pkg_name));
    fs::write(&kdl_path, template_content).map_err(LuppoError::IoError)?;

    let service_kdl = include_str!("../luppo-template/comar/service.kdl");
    let package_kdl = include_str!("../luppo-template/comar/package.kdl");
    let pakhandler_kdl = include_str!("../luppo-template/comar/pakhandler.kdl");

    fs::write(dir_path.join("comar/service.kdl"), service_kdl).map_err(LuppoError::IoError)?;
    fs::write(dir_path.join("comar/package.kdl"), package_kdl).map_err(LuppoError::IoError)?;
    fs::write(dir_path.join("comar/pakhandler.kdl"), pakhandler_kdl)
        .map_err(LuppoError::IoError)?;

    let template_patch = include_str!("../luppo-template/files/template.patch");
    fs::write(dir_path.join("files/template.patch"), template_patch).map_err(LuppoError::IoError)?;
