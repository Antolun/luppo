...
pub fn parse_kdl_spec_from_str(content: &str) -> Result<PisiSpec, String> {
    ...
    let mut spec = PisiSpec::default();
    let nodes = if doc.nodes().len() == 1 && doc.nodes()[0].name().to_string() == "PisiPackage" {
...
mod tests {
    ...
    fn parse_kdl(input: &str) -> PisiSpec {
...
"#;
    ...
    fn test_parse_error_shows_line_number() {
        let err = parse_kdl_spec_from_str(r#"PisiPackage {
    Source { Name "test" }
    Package {
        Name "test"
        Actions {
